...
inline void* __temporary_allocator(Allocator_Mode mode, void *allocatorData, size_t size, void *oldMemory, size_t oldSize, s32 options) {
    ...
    case Allocator_Mode::RESIZE:
    {
        if (storage->Occupied + size > storage->Size) {
            ...
            if (__context->Allocator.Function == __temporary_allocator || __context->Allocator.Data == __temporary_allocator_data) {
                switched = true;
...
#define TEMPORARY_STORAGE_MARK_SCOPE size_t TEMP_VAR_GEN(__LINE__) = temporary_storage_get_mark(); \
                                     defer { temporary_storage_set_mark(TEMP_VAR_GEN(__LINE__)); }

#define TEMPORARY_ALLOC Allocator_Closure { __temporary_allocator, __temporary_allocator_data }
