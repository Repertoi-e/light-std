...
struct Value {
    ...
    template <typename T>
    static void _format_custom_arg(const void *arg, Format_Context &context) {
        Formatter<T> f;
...
struct Format_Arguments {
    ...
    template <typename... Args>
    Format_Arguments(const Format_Arguments_Store<Args...> &store) : _Types((u64) store.TYPES), _Args(store._Data) {}
