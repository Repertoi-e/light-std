...
inline f64 os_time_to_seconds(time_t time) { return (f64)time / Win32PerformanceFrequency.QuadPart; }
...
inline mark_as_leak os_get_env_result os_get_env(string name, bool silent) {
    ...
    DWORD bufferSize = 65535;  // Limit according to
    ...
    if (r == 0 && GetLastError() == ERROR_ENVVAR_NOT_FOUND) {
        if (!silent) { platform_report_error(tprint("Couldn't find environment variable with value \"{}\"", name)); }
        return {"", false};
    }
...
inline void os_remove_env(string name) { WIN32_CHECK_BOOL(r, SetEnvironmentVariableW(utf8_to_utf16(name), null)); }
...
inline void atexit(void (*function)(void)) {
    ...
    PUSH_ALLOC(PERSISTENT) { add(S->ExitFunctions, function); }
