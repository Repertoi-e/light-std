...
#define GET_READONLY_EXISTING_HANDLE(x, fail)                                                                                                                                    \
    CREATE_FILE_HANDLE_CHECKED(x, CreateFileW(platform_utf8_to_utf16(path), GENERIC_READ, FILE_SHARE_READ | FILE_SHARE_WRITE, null, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL), \
                               fail);                                                                                                                                            \
    defer(CloseHandle(x));

// == is_file() || is_directory()
inline bool path_exists(string path) {
    HANDLE file = CreateFileW(platform_utf8_to_utf16(path), GENERIC_READ, FILE_SHARE_READ, null, OPEN_EXISTING, FILE_FLAG_BACKUP_SEMANTICS, null);
...
inline bool path_is_file(string path) {
    HANDLE file = CreateFileW(platform_utf8_to_utf16(path), 0, 0, null, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, null);
    if (file == INVALID_HANDLE_VALUE) return false;
...
inline bool path_is_symbolic_link(string path) {
    ...
    if (attribs != INVALID_FILE_ATTRIBUTES) { return (attribs & FILE_ATTRIBUTE_REPARSE_POINT) != 0; }
    return false;
}
...
inline s64 path_file_size(string path) {
    ...
    CREATE_FILE_HANDLE_CHECKED(file, CreateFileW(platform_utf8_to_utf16(path), GENERIC_READ, FILE_SHARE_READ, null, OPEN_EXISTING, FILE_ATTRIBUTE_READONLY, null), 0);
    defer(CloseHandle(file));
...
inline bool path_move(string path, string dest, bool overwrite) {
    ...
    if (path_is_directory(dest)) {
        ...
        return MoveFileExW(platform_utf8_to_utf16(path), platform_utf8_to_utf16(p), MOVEFILE_WRITE_THROUGH | MOVEFILE_COPY_ALLOWED | (overwrite ? MOVEFILE_REPLACE_EXISTING : 0));
    }
    return MoveFileExW(platform_utf8_to_utf16(path), platform_utf8_to_utf16(dest), MOVEFILE_WRITE_THROUGH | MOVEFILE_COPY_ALLOWED | (overwrite ? MOVEFILE_REPLACE_EXISTING : 0));
}
...
inline bool path_create_hard_link(string path, string dest) {
    ...
    return CreateHardLinkW(platform_utf8_to_utf16(dest), platform_utf8_to_utf16(path), null);
}
...
#define CHECK_FIND_NEXT(call)                                                                                                   \
    if (!call) {                                                                                                                \
        if (GetLastError() != ERROR_NO_MORE_FILES) { windows_report_hresult_error(HRESULT_FROM_WIN32(GetLastError()), #call); } \
        if (walker.Handle != INVALID_HANDLE_VALUE) { WIN32_CHECK_BOOL(r, FindClose((HANDLE)walker.Handle)); }                   \
                                                                                                                                \
...
inline void path_walk_recursively_impl(string path, string first, array<string> ref result) {
    ...
    while (true) {
        ...
        if (path_is_directory(p)) { path_walk_recursively_impl(p, first, result); }
    }
}
