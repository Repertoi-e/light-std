...
// This makes the program compile faster, but that's not the actual reason we do
// this... In reality Windows.h complains about not having symbols normally
// found in the CRT! :AvoidSTL: We avoid including any files from the C++
// standard library.
//
...
//
// HRESULT_FROM_WIN32(x) used to be a macro, however we now run it as an inline
...
always_inline HRESULT HRESULT_FROM_WIN32(unsigned long x) { return (HRESULT)x <= 0 ? (HRESULT)x : (HRESULT)(x & 0x0000FFFF | FACILITY_WIN32 << 16 | 0x80000000); }
...
NTSYSAPI ULONGLONG VerSetConditionMask(ULONGLONG ConditionMask, DWORD TypeMask, BYTE Condition);
...
ATOM RegisterClassExW(const WNDCLASSEXW *Arg1);
...
HWND CreateWindowExW(DWORD dwExStyle, LPCWSTR lpClassName, LPCWSTR lpWindowName, DWORD dwStyle, int X, int Y, int nWidth, int nHeight, HWND hWndParent, HMENU hMenu,
                     HINSTANCE hInstance, LPVOID lpParam);

BOOL ShowWindow(HWND hWnd, int nCmdShow);

BOOL WINAPI PeekMessageW(LPMSG lpMsg, HWND hwnd, UINT uMsgFilterMin, UINT uMsgFilterMax, UINT wRemoveMsg);
...
BOOL WINAPI    PostThreadMessage(DWORD dwThreadId, UINT Msg, WPARAM wParam, LPARAM lParam);
...
typedef enum _HEAP_INFORMATION_CLASS { HeapCompatibilityInformation, HeapEnableTerminationOnCorruption, HeapOptimizeResources } HEAP_INFORMATION_CLASS;
...
#define HEAP_memset0               0x00000008
...
BOOL SetCurrentDirectoryW(LPCTSTR lpPathName);
...
HANDLE CreateFileW(LPCWSTR lpFileName, DWORD dwDesiredAccess, DWORD dwShareMode, LPSECURITY_ATTRIBUTES lpSecurityAttributes, DWORD dwCreationDisposition,
...
#define IMAGE_FIRST_SECTION(ntheader) \
    ((PIMAGE_SECTION_HEADER)((ULONG_PTR)ntheader + offset_of(IMAGE_NT_HEADERS64, OptionalHeader) + ((PIMAGE_NT_HEADERS64)(ntheader))->FileHeader.SizeOfOptionalHeader))
...
//
// The following are helper macros for error reporting.
//
...
#define WIN32_CHECK_HR(result, call) \
    u32 result = call;               \
    if (result < 0) ARCHE_NAMESPACE::windows_report_hresult_error(result, #call);

// If the returned is false, reports an error.
#define WIN32_CHECK_BOOL(result, call) \
    auto result = call;                \
    if (!result) ARCHE_NAMESPACE::windows_report_hresult_error(HRESULT_FROM_WIN32(GetLastError()), #call);
...
// WIN32_CHECK_HR is that in Release configuration, the macro expands to just
// the call (no error checking).
#if !defined NDEBUG
