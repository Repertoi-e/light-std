...
inline void lock(mutex *m) { pthread_mutex_lock((pthread_mutex_t *)m->Handle); }

inline bool try_lock(mutex *m) { return pthread_mutex_trylock((pthread_mutex_t *)m->Handle); }

inline void unlock(mutex *m) { pthread_mutex_unlock((pthread_mutex_t *)m->Handle); }
...
inline void free_condition_variable(condition_variable *c) {
    if (c) { pthread_cond_destroy((pthread_cond_t *)c->Handle); }
}
...
inline void do_wait(condition_variable *c, mutex *m) {
    ...
    if (result != 0) { report_warning_no_allocations("Error in pthread_cond_wait"); }
}
}  // namespace internal

inline void notify_one(condition_variable *c) { pthread_cond_signal((pthread_cond_t *)c->Handle); }

inline void notify_all(condition_variable *c) { pthread_cond_broadcast((pthread_cond_t *)c->Handle); }
...
inline void *thread_wrapper_function(void *data) {
    ...
    // thread-safe).
    if (ti->ParentWasUsingTemporaryAllocator) { newContext.Alloc = TemporaryAllocator; }
...
inline void terminate(thread t) {
    if (t.Handle) { pthread_cancel((pthread_t)t.Handle); }
}
