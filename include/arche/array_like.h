...
constexpr bool should_array_be_treated_as_array_like() {
    if constexpr (does_array_have_flag<T>) { return T::TREAT_AS_ARRAY_LIKE; }
    return true;
}
...
template <typename T>
concept any_dynamic_array_like = internal::is_dynamic_array_like<remove_cv_t<T>>;
...
//
// This function checks if the index is in range if ARCHE_ARRAY_BOUNDS_CHECK is
// defined.
...
//
always_inline s64 translate_negative_index(s64 index, s64 length, bool tolerateOnePastLast = false);
...
always_inline s64 translate_negative_index(s64 index, s64 length, bool tolerateOnePastLast) {
    if (index < 0) {
        s64 r = length + index;
...
void check_debug_memory(any_dynamic_array_like auto no_copy arr) {
    ...
    if (arr.Allocated) { assert(debug_memory_list_contains((allocation_header *)arr.Data - 1)); }
#endif
}
...
auto *insert_at_index(Arr ref arr, s64 index, array_data_t<Arr> no_copy element) {
    ...
    if (offset < arr.Count) { memmove(where + 1, where, (arr.Count - offset) * sizeof(*where)); }
    *where = element;
    ++arr.Count;
    return where;
}
...
auto *insert_at_index(Arr ref arr, s64 index, const array_data_t<Arr> *ptr, s64 size) {
    ...
    s64   offset = translate_negative_index(index, arr.Count, true);
    auto *where  = arr.Data + offset;
    if (offset < arr.Count) { memmove(where + size, where, (arr.Count - offset) * sizeof(*where)); }
...
void remove_unordered_at_index(any_dynamic_array_like auto ref arr, s64 index) {
    ...
    if (offset != arr.Count - 1) { *where = *(arr.Data + arr.Count - 1); }
    --arr.Count;
}
...
void replace_range(Arr ref arr, s64 begin, s64 end, Arr2 no_copy replace) {
    ...
    if (diff > 0) { maybe_grow(arr, diff); }

    auto where = arr.Data + targetBegin;

    // Make space for the new elements
    memmove(where + replace.Count, where + whereSize, (arr.Count - targetBegin - whereSize) * sizeof(*where));
