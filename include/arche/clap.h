...
enum clap_arg_type { CLAP_ARG_STRING, CLAP_ARG_INT, CLAP_ARG_FLOAT, CLAP_ARG_BOOL };
...
// Parser functions
clap_parse_result clap_parse(clap_parser & parser, int argc, char **argv);
clap_parse_result clap_parse_args(clap_parser & parser, array<string> args);
void              clap_print_help(clap_parser & parser);
void              clap_print_version(clap_parser & parser);

// Result access functions
string clap_get_string(clap_parse_result & result, string key);
s64    clap_get_int(clap_parse_result & result, string key);
f64    clap_get_float(clap_parse_result & result, string key);
bool   clap_get_bool(clap_parse_result & result, string key);
bool   clap_has_arg(clap_parse_result & result, string key);

// Helper functions
clap_arg *clap_find_arg_by_short(clap_parser & parser, string short_name);
clap_arg *clap_find_arg_by_long(clap_parser & parser, string long_name);
...
inline clap_arg clap_arg_option_opt(string name, clap_option_desc opts) {
    clap_arg arg      = {};
    arg.name          = name;
...
inline clap_arg clap_arg_positional_opt(string name, clap_positional_desc opts) {
    clap_arg arg      = {};
    arg.name          = name;
    arg.value_name    = opts.value_name.Count ? opts.value_name : name;
    arg.help_text     = opts.help_text;
...
inline clap_arg clap_arg_flag_opt(string name, clap_flag_desc opts) {
    clap_arg arg      = {};
    arg.name          = name;
    arg.value_name    = name;
    arg.help_text     = opts.help_text;
...
inline clap_arg *clap_find_arg_by_short(clap_parser & parser, string short_name) {
    For(parser.arguments) {
        if (strings_match(it.short_name, short_name)) { return &it; }
    }
    return null;
}

inline clap_arg *clap_find_arg_by_long(clap_parser & parser, string long_name) {
    For(parser.arguments) {
        if (strings_match(it.long_name, long_name)) { return &it; }
    }
    return null;
}

inline bool clap_parse_value(string value_str, clap_arg_type type, clap_argument_value *out_value) {
    out_value->type = type;

    switch (type) {
        case CLAP_ARG_STRING: out_value->value = value_str; return true;
        ...
        case CLAP_ARG_BOOL: {
            if (strings_match(value_str, "true") || strings_match(value_str, "1") || strings_match(value_str, "yes") || strings_match(value_str, "on")) {
                out_value->value = true;
                return true;
            } else if (strings_match(value_str, "false") || strings_match(value_str, "0") || strings_match(value_str, "no") || strings_match(value_str, "off")) {
...
inline void clap_print_help(clap_parser & parser) {
    ...
    if (has_options) { print(" {!GRAY}[options]{!}"); }

    print("\n");

    if (parser.about_text.Count) { print("\n{}\n", parser.about_text); }
    ...
    For(parser.arguments) {
        if (it.is_positional) {
            ...
            if (it.help_text.Count) { print("  {}", it.help_text); }
            print("\n");
        }
    }
    ...
    if (has_options) {
        ...
        For(parser.arguments) {
            if (!it.is_positional) {
                print("  ");
                if (it.short_name.Count) { print("-{}, ", it.short_name); }
                if (it.long_name.Count) { print("--{}", it.long_name); }
                if (it.arg_type != CLAP_ARG_BOOL) { print(" <{}>", it.value_name); }
                if (it.help_text.Count) { print("  {}", it.help_text); }
                if (it.default_val.Count) { print(" (default: {})", it.default_val); }
                print("\n");
            }
        }

        print("  -h, --help     Show this help message\n");
        if (parser.version_text.Count) { print("  -v, --version  Show version information\n"); }
    }
}

inline void clap_print_version(clap_parser & parser) {
...
inline clap_parse_result clap_parse(clap_parser & parser, int argc, char **argv) {
    array<string> args = os_parse_arguments(argc, argv);
    return clap_parse_args(parser, args);
}

inline clap_parse_result clap_parse_args(clap_parser & parser, array<string> args) {
                ...
                for (s64 j = 0; j < shorts_len; ++j) {
                    ...
                    if (found_arg->action == CLAP_ACTION_SET_TRUE) {
                        ...
                        if (vp && vp->type == CLAP_ARG_INT) {
                            // extract existing
                            s64 existing = 0;
                            vp->value.visit(match{[&existing](s64 v) { existing = v; }, [](auto) {}});
                            new_val = existing + 1;
                        }
    ...
    For(parser.arguments) {
        ...
        if (!val) {
            ...
            if (it.default_val.Count) {
                ...
                if (clap_parse_value(it.default_val, it.arg_type, &default_value)) { set(result.values, it.name, default_value); }
            }
        }
    }
...
inline string clap_get_string(clap_parse_result & result, string key) {
    auto [kp, val] = search(result.values, key);
    if (val && val->type == CLAP_ARG_STRING) {
        string ret_val = "";
        val->value.visit(match{[& ret_val](string value) { ret_val = value; }, [](auto) {}});
        return ret_val;
    }
    return "";
}

inline s64 clap_get_int(clap_parse_result & result, string key) {
    auto [kp, val] = search(result.values, key);
    if (val && val->type == CLAP_ARG_INT) {
        s64 ret_val = 0;
        val->value.visit(match{[& ret_val](s64 value) { ret_val = value; }, [](auto) {}});
        return ret_val;
    }
    return 0;
}

inline f64 clap_get_float(clap_parse_result & result, string key) {
    auto [kp, val] = search(result.values, key);
    if (val && val->type == CLAP_ARG_FLOAT) {
        f64 ret_val = 0.0;
        val->value.visit(match{[& ret_val](f64 value) { ret_val = value; }, [](auto) {}});
        return ret_val;
    }
    return 0.0;
}

inline bool clap_get_bool(clap_parse_result & result, string key) {
    auto [kp, val] = search(result.values, key);
    if (val && val->type == CLAP_ARG_BOOL) {
        bool ret_val = false;
        val->value.visit(match{[& ret_val](bool value) { ret_val = value; }, [](auto) {}});
        return ret_val;
    }
    return false;
}

inline bool clap_has_arg(clap_parse_result & result, string key) { return search(result.values, key).Value != null; }
