...
// and a dynamic integer which grows dynamically on the heap as it needs more
// range.
//
...
//
// We use u32s for storing single digits, each digit has range from -2^30 to
// 2^30. When implementing operations we use grade school math, but treat each
// digit of the integer as in base 2^30 instead of base 10.
//
...
// @Cleanup These are leaked globally
using digit  = u32;
using sdigit = s32;
...
//
// When doing arithmetic operations we allocate a new big_integer (we don't do
// the work in place). This is because: 1. for most operations we need to
// allocate a seperate buffer to store the result anyway; 2. sometimes you don't
// want to discard the old result.
//
...
// Note: Big integers have minimum storage of 5 digits (and are bigger than
// u128). For smaller values you'd better be using the smaller integer types (8
// to 128 bit).
...
inline bool operator==(big_integer lhs, big_integer rhs) { return compare(lhs, rhs) == 0; }

inline bool operator!=(big_integer lhs, big_integer rhs) { return compare(lhs, rhs) != 0; }

inline auto operator<=>(big_integer lhs, big_integer rhs) { return compare(lhs, rhs); }
...
inline big_integer make_big_integer(s64 initialDigits) {
    big_integer b;
    ensure_digits(b, initialDigits);
    return b;
}

inline big_integer make_big_integer_and_set_size(s64 initialDigits) {
...
inline big_integer make_big(is_integral auto v) {
    big_integer b;
    assign(b, v);
    return b;
}
...
inline digit v_iadd(digit *x, s64 m, digit *y, s64 n) {
    assert(m >= n);
...
inline digit v_isub(digit *x, s64 m, digit *y, s64 n) {
    assert(m >= n);
...
inline big_integer x_add(big_integer *a, big_integer *b) {
...
inline big_integer x_sub(big_integer *a, big_integer *b) {
    s64 sizea = abs(a->Size), sizeb = abs(b->Size);
    s32 sign = 1;
...
inline big_integer x_mul(big_integer lhs, big_integer rhs) {
...
inline s64 KARATSUBA_CUTOFF        = 70;
inline s64 KARATSUBA_SQUARE_CUTOFF = 2 * KARATSUBA_CUTOFF;
...
inline void kmul_split(big_integer n, s64 size, big_integer *high, big_integer *low) {
...
inline big_integer k_lopsided_mul(big_integer *a, big_integer *b) {
    s64 sizea = abs(a->Size), sizeb = abs(b->Size);
...
inline big_integer k_mul(big_integer *a, big_integer *b) {
    s64 sizea = abs(a->Size), sizeb = abs(b->Size);
    ...
    // @Speed We treat big integers as immutable. That has serious performance
    // implications. Also the (a == b) comparison here..
    ...
    // 3. Compute al*bl, and copy into result at 0. Note that this can't overlap
    // with #2.
    ...
    // to start with:
    // 1. For any integer i, i = c(i/2) + f(i/2). In particular, sizeb =
    // c(sizeb/2) + f(sizeb/2).
    // 2. shift = f(sizeb/2)
    // 3. sizea <= sizeb
    // 4. Since we call k_lopsided_mul if sizea*2 <= sizeb, sizea*2 > sizeb in
    // this
    ...
    //
    // If sizea < sizeb, then we're asking whether sizea digits >= f(sizeb/2)
    // digits + 2 bits. By #4, sizea is at least f(sizeb/2)+1 digits, so this in
    // turn reduces to whether 1 digit is enough to hold 2 bits.  This is so since
    // SHIFT=15 >= 2.
    //
    // If sizea == sizeb, then we're asking whether sizeb digits is enough to hold
    // c(sizeb/2) digits + 2 bits, or equivalently (by #1) whether f(sizeb/2)
    // digits is enough to hold 2 bits.  This is so if sizeb >= 2, which holds
    // because sizeb >= KARATSUBA_CUTOFF >= 2.
    //
...
inline digit inplace_divrem1(digit *pout, const digit *pin, s64 size, digit n) {
    double_digit rem = 0;
...
inline div_result x_divrem(big_integer a, big_integer b) {
    // We follow Knuth [The Art of Computer Programming, Vol. 2 (3rd edn.),
    // section 4.3.1, Algorithm D]. This divides an n-word dividend by an m-word
    // divisor and gives an n-m+1-word quotient and m-word remainder.
    ...
    while (j--) {
        ...
        For(range(sizeb)) {
            ...
            z = (sdigit)get_digit(na, it + j) + zhi - (sdouble_digit)qhat * (sdouble_digit)get_digit(nb, it);
...
inline void v_complement(digit *z, digit *a, s64 m) {
    digit carry = 1;
    For(range(m)) {
        carry += a[it] ^ MASK;
        z[it] = carry & MASK;
...
inline big_integer bitwise(big_integer lhs, byte op, big_integer rhs) {
...
inline big_integer operator&(big_integer lhs, big_integer rhs) { return bitwise(lhs, '&', rhs); }
...
inline big_integer invert(big_integer b) {
...
inline big_integer lshift(big_integer lhs, s64 n) {
    ...
    For(range(wordshift)) { set_digit(result, it, 0); }
...
inline div_result divrem(big_integer lhs, big_integer rhs) {
    ...
    if ((lhs.Size < 0) != (rhs.Size < 0)) { result.Q.Size = -result.Q.Size; }
    if (lhs.Size < 0 && result.R.Size != 0) { result.R.Size = -result.R.Size; }
    return result;
}

inline div_result divmod(big_integer lhs, big_integer rhs) {
...
inline s32 compare(big_integer a, big_integer b_) {
...
inline big_integer operator+(big_integer lhs, big_integer rhs) {
    if (lhs.Size < 0) {
        if (rhs.Size < 0) {
...
inline big_integer operator*(big_integer lhs, big_integer rhs) {
    ...
    if ((lhs.Size ^ rhs.Size) < 0 && result.Size) { result.Size = -result.Size; }
