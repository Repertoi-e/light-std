...
inline bool ascii_is_hex_digit(char x) { return (x >= '0' && x <= '9') || (x >= 'a' && x <= 'f') || (x >= 'A' && x <= 'F'); }

inline bool ascii_is_space(char x) { return (x >= 9 && x <= 13) || x == 32; }

inline bool ascii_is_blank(char x) { return x == 9 || x == 32; }

inline bool ascii_is_alpha(char x) { return (x >= 65 && x <= 90) || (x >= 97 && x <= 122); }
...
//
// If ARCHE_ARRAY_BOUNDS_CHECK is defined this fails if we go out of bounds.
//
// @Speed @TODO Vectorize for large strings
inline const char *utf8_get_pointer_to_cp_at_translated_index(const char *str, s64 byteLength, s64 index) {
    ...
    For(range(index)) {
        ...
        if (str >= end) { assert(false && "Out of bounds"); }
...
// _outByteLength_ (assumes there is enough space).
inline void utf32_to_utf8(const code_point *str, char *out, s64 *outByteLength) {
    s64 byteLength = 0;
    while (*str) {
...
inline s64 utf8_find_invalid(const char *str, s64 byteLength) {
    ...
    while (p < end) {
        ...
        if (cpSize <= 0) return p - str;
        if ((end - p) < cpSize) return p - str;  // truncated sequence
        if (!utf8_is_valid_cp(p)) return p - str;
        p += cpSize;
    }
    return -1;
}
...
inline bool utf8_segment_nfd(const char *&p, const char *end, stack_array<code_point, 1024> &segBuf, s64 &segN) {
    segN = 0;
    if (p >= end) return false;  // Decode first code point (starter)
    ...
    auto decompose_full_into_seg = [&](code_point cp0) {
        constexpr s64                      MAX_STACK      = 64;
        constexpr s64                      MAX_TMP_DECOMP = 8;
        ...
        while (sp > 0) {
            ...
            if (n > 1) {
                ...
                if (segN < 1024) segBuf.Data[segN++] = x;
            }
        }
    };
    ...
    while (p < end) {
        ...
        if (sz2 <= 0 || p + sz2 > end) return false;
        if (!utf8_is_valid_cp(p)) return false;
        code_point c  = utf8_decode_cp(p);
        u8         cc = unicode_combining_class(c);
        if (cc == 0) break;  // next segment starts
...
inline void check_debug_memory(string const& s) {
    ...
    // allocated with arche's allocators, but then you called a function that
    ...
    //
    if (s.Allocated) { assert(debug_memory_list_contains((allocation_header *)s.Data - 1)); }
#endif
}
...
inline void insert_at_index(string & s, s64 index, const char *str, s64 size) {
    ...
    if (offset < s.Count) { memmove(where + size, where, (s.Count - offset) * sizeof(*where)); }
...
inline code_point get(string str, s64 index) {
    if (index < 0) {
        ...
        // If ARCHE_ARRAY_BOUNDS_CHECK is defined:
...
inline s64 search(string str, delegate<bool(code_point)> predicate, search_options options) {
    ...
    if (options.Start >= len || options.Start < -len) return -1;
...
inline void replace_range_bytes(string & str, s64 byte_begin, s64 byte_end, string replace) {
    ...
    memmove(where + replace.Count, where + whereSize, (str.Count - byte_end) * sizeof(*where));
