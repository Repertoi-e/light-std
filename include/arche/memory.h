...
//
// By default we do some extra work when allocating to make it easier to catch
// memory related bugs. That work is measurable in performance so we don't want
// to do it in Release configuration. If you want to disable it for
// Debug/DebugOptimized as well, define FORCE_NO_DEBUG_MEMORY. You can read the
// comments in this file (around where DEBUG_MEMORY is mentioned) and see what
// extra safety stuff we do.
//
...
// several) large blocks (pools) from which smaller blocks are used for
// allocations. This forces you to think about the memory layout. The hope is
// that faster software is produced because modern computers suffer a lot from
// cache misses.
//
// C++ can be used as a low-level language. Usually modern high-level languages
// put much of the memory management behind walls of abstraction. Hardware has
// gotten blazingly fast, but software has deteriorated. Modern CPUs can make
// billions of calculations per second, but reading even ONE byte from RAM can
// take hundreds of clock cycles (if the memory is not in the cache). You MUST
// think about the cache if you want to write fast software.
//
// Once you start thinking about the cache you start programming in a data
// oriented way. You don't try to model the real world in the program, but
// instead structure the program in the way that the computer will work with the
// data. Data that is processed together should be close together in memory.
//
// 2007, "What Every Programmer Should Know About Memory", Ulrich Drepper
// https://people.freebsd.org/~lstewart/articles/cpumemory.pdf
//
// And that's why we should remove some of the abstraction. We should think
...
//
// free<T>:
// - Calls destructors. Normally we are strongly against destructors. Explicit
// code is better
//   so to implement "destructor functionality" (uninitializing the type,
//   freeing members, etc.) you should provide a function, for example:
//
...
//          }
//
//   You may say that this is just a renamed destructor. But this doesn't run
//   hiddenly (when exiting a scope or a function). Sometimes you actually want
//   to do that, in that case you can do    defer(free(s));    Which is a useful
//   macro that calls any statements at the end of a scope (emulates a
//   destructor).
//
//   But free still calls destructors, just in case.
//
//
// For more philosophy (like why we don't like exceptions, copy or move
// constructors) you can look at the type policy (:TypePolicy: in common.h).

//
// Technical notes:
//
// For each allocation we reserve a bit of space before a block to store a
// header with information (the size of the allocation, the alignment, the
// allocator with which it was allocated, and debugging info if DEBUG_MEMORY is
// defined - see comments in allocator.h).
//              (@TODO * this overhead should become smaller, right now we
//              haven't bothered yet!)
//
// There is one big assumption we make:
//   Your types are "trivially copyable" which means that they can be copied
//   byte by byte to another place and still work. We don't call copy/move
//   constructors when e.g. reallocating. This aligns with our :TypePolicy: (see
//   common.h).
//
// We are following a data oriented design with this library.
// e.g. our array<> type also expects type to be simple data. If you need extra
// logic when copying memory, implement that explicitly (not with a copy
// constructor). We do that to avoid C++ complicated shit that drives sane
// people insane.
//
...
//
// Allocators in this library are a pair of a function pointer and a data
// pointer. The single function is responsible for doing all stuff (ALLOCATE,
// RESIZE, FREE, FREE_ALL). The data pointer can point to any custom structure
// or data the allocator needs in order to do it's job.
//

enum class allocator_mode { ALLOCATE, RESIZE, FREE, FREE_ALL };
...
//      Note: * Implementing FREE_ALL is not a requirement, some allocators
//      can't support this by design.
//
...
//
// The last pointer to u64 is reserved for options. The allocator
// implementation decides how to treat those. We just provide a way to pass
// those. Note: The LEAK option means that bit number 64 is reserved for
// special use. That means that the maximum integer you have to work with is
// the bottom 63 bits of the options. We expect people to use this as a bit
// field anyway.
//
//
...
//
//     In the second case we automatically handle allocating a new block and
...
//     unaligned pointer we can freely bump it. The information about the
...
// This is a hack that's used to get around Context's and TemporaryAllocator's
// constructors getting fired and overriding its allocator (which may have
// potentially been set by other global constructors).
...
template <non_void T>
T *arche_allocate_impl(s64 count, allocator alloc, u32 alignment, u64 options, source_location loc);

template <non_void T>
    requires(!is_const<T>)
T *arche_reallocate_impl(T *block, s64 newCount, u64 options, source_location loc);

template <non_void T>
    requires(!is_const<T>)
void arche_free_impl(T *block, u64 options, source_location loc);

// These handle alignment, populating the allocation header and debug memory
// stuff.
void *general_allocate(allocator alloc, s64 userSize, u32 alignment, u64 options, source_location loc = source_location::current());
...
void *general_reallocate(void *ptr, s64 newUserSize, u64 options, source_location loc = source_location::current());
...
struct allocate_options {
    // How many items to allocate, historically we provided a seperate
    // allocate_array routine, but we merged it.
    s64 Count = 1;
...
T *malloc(allocate_options options = {}, source_location loc = source_location::current()) {
    return arche_allocate_impl<T>(options.Count, options.Alloc, options.Alignment, options.Options, loc);
}
...
// There is no reason to initialize everything to zero after malloc has called
// T's constructors. Either modify your type to initialize it's members to
// zero in the constructor or call the standard calloc routine which is
// outside of this namespace. template <non_void T> T *calloc(allocate_options
// options = {}, source_location loc = source_location::current()) { }

// If DEBUG_MEMORY is defined, calling reallocate on a block that is already
// freed panics the program and gives information about the site.
//
// Note: We don't support "non-trivially copyable" types (types that can have
// logic in the copy constructor). We assume your type can be copied to
// another place in memory and just work. We assume that the destructor of the
// old copy doesn't invalidate the new copy. We don't do destructors in this
// library but we still call them here, just in case.
template <non_void T>
T *realloc(T *block, reallocate_options options, source_location loc = source_location::current()) {
    return arche_reallocate_impl<T>(block, options.NewCount, options.Options, loc);
}

// If DEBUG_MEMORY is defined, calling free on a block that is already
// freed panics the program and gives information about the site.
//
// We don't do destructors in this library but we still call them here, just
// in case. If T is non-scalar we call the destructors on the objects in the
// memory block (the destructor is determined by T, like in the C++ delete, so
// make sure you pass a correct pointer type).
//
// The allocation header contains the size of the allocation, that's how we
// know on how many elements to call destructors (assuming the T is valid).
// new[] and delete[] are implemented the same way (they save an integer
// before the returned block). That's why it's dangerous to mix new and
// delete[] and new[] and delete.
//
// However we only have one type of free here.
template <non_void T>
    requires(!is_const<T>)
void free(T *block, u64 options = 0, source_location loc = source_location::current()) {
    arche_free_impl(block, options, loc);
}
...
// - Different algorithms for allocation, e.g. stb_malloc implements the TLSF
// algorithm for O(1) allocation
//
...
// - Don't take this as boilerplate which you need to write so you can then
// use this general purpose
//   allocator for everything. Once you go beyond the quick and dirty
//   sketching of your code, look for places which can benefit from a more
//   specialized allocator (arena allocator, pool allocator, etc.)
//
...
//
// Two-Level Segregated Fit memory allocator implementation. Wrapper around
// tlsf.h/cpp (in vendor folder), written by Matthew Conte (matt@baisoku.org).
...
// * Low fragmentation
//
inline void *tlsf_allocator(allocator_mode mode, void *context, s64 size, void *oldMemory, s64 oldSize, u64 options) {
    assert(context);
...
inline void tlsf_allocator_add_pool(tlsf_allocator_data *data, void *block, s64 size) {
    if (!data->State) {
...
inline void tlsf_allocator_remove_pool(tlsf_allocator_data *data, void *block) { tlsf_remove_pool(data->State, block); }
...
struct arena_allocator_data {
    ...
    s64 AutomaticBlockSize = 0;  // You can change this before using the allocator, by default it's 128_MiB
};
...
// the entire thing (with FREE_ALL) when you are sure nobody uses the memory
// anymore. Note that free_all doesn't free the added block, but instead
// resets its pointer to the beginning of the buffer.
//
// The arena allocator doesn't handle overflows (when the block doesn't have
...
#else
            data->AutomaticBlockSize = 128_MiB;
#endif
        }
...
// Use this to provide more space in the pool allocator. Also inits the first
...
//   lower bit (e.g. properly aligned pointers to types other than char are
//   not odd).
// - Large byte values are less typical and are useful for finding bad
// addresses.
// - Atypical values are good because they typically cause early detection in
// code.
...
// The returned pointer is guaranteed to be aligned to the specified
// alignment, we do that by padding this structure. Info about that is saved
// in the header itself. Right now this uses 32 bytes when DEBUG_MEMORY is not
// defined, 96 bytes when storing debug info.
//
// @TODO: This may be too big of an overhead. We can make this smaller!
//        We can split allocations into small/medium/large, and use different
//        headers (like https://nothings.org/stb/stb_malloc.h).
//
struct allocation_header {
    // The allocator used when allocating the memory. We need this when
    // resizing/freeing in order to call the right allocator procedure. By
    // design, we can't get rid of this.
    allocator Alloc;

    // The size of the allocation (NOT including the size of the header and
    // padding). By design, we can't get rid of this (but we can make this
    // smaller for medium sized allocations by packing a different header).
    s64 Size;

#if defined DEBUG_MEMORY
    // This is another guard to check that the header is valid.
    // This points to "(allocation_header *) p + 1" (the pointer we return after
    // the allocation).
    void *DEBUG_Pointer;
#endif
    ...
    //
    // User requests allocation of _size_. The underlying allocator is called
    // with
    //     _size_ + sizeof(allocation_header) + (sizeof(allocation_header) %
    //     alignment)
    //
    // The result:
    //   ...[..Alignment
    ...
    // There may be padding after this member, but we treat this region as
    // "(allocation_header *) p + 1 - 4 bytes". This doesn't matter since we
    // just need AT LEAST 4 bytes free.
    char DEBUG_NoMansLand[NO_MANS_LAND_SIZE];

    // After the whole memory block we follow again by NO_MANS_LAND_SIZE bytes
    // of no man's land again. We use that to detect buffer overruns.
#endif
};
...
struct debug_memory_node {
    ...
    // to disable this treatment explicitly (with the ALLOW_CROSS_THREAD_FREE
    // allocation flag). See :AllocationFlags:
    //
    u32 TID;

    // @TODO: We don't support cross-thread freeing right now because
    // our debug list of nodes is not thread safe. We can make a thread-safe
    // queue for handling these cases. bool MarkedAllowCrossThreadFree;

    //
    // This ID is useful for debugging bugs related to allocations.
    // Every allocation has an unique number == to the ID of the previous
    // allocation + 1. The allocation count is unique to each thread in order to
    ...
    //
    // Currently, if you set a breakpoint with a specific ID, you may need to
    // skip a few breaks from other threads, or look at the TID, but TID is
    // random, so...
    ...
    // When realloc() is called we check if the block can be directly resized in
    // place (using allocation_mode::RESIZE). If not, we allocate a new block
    // and copy the information there. In both cases the ID above stays the same
    // and RID gets incremented. This always starts at 0.
    //
    // This helps debug memory allocations that are reallocated often
    // (strings/arrays, etc.)
    //
    s64 RID;

    //
    // We mark the source of the (re)allocation if such information was
    // provided. On subsequent reallocations we overwrite this.
    //
    // When allocating with the default malloc/calloc/realloc (the non-templated
    // extern "C" versions which are replacements for the standard library
    // functions) we don't get a very useful FileName and FileLine (we get the
    // ones from the wrapper). This also applies when an allocation is made with
    // C++ new.
    //
    // @TODO Remove this and instead save a call stack (optional!, that's a lot
    // of info).
    //
    // To debug allocations you can try to use the _ID_ and set a breakpoint.
    // See info above.
    //
    source_location AllocatedAt;

    //
    // When calling general_free() we free the block with the allocator
    // implementation but keep the node live in the list. We do this in order to
    // detect freeing the same pointer twice. If the program requests a new
    // block and the allocator implementation returns the same memory address
    // then we reuse this node and clear the flag.
    //
    // Not freeing nodes might accummulate some memory overhead. @TODO Provide a
    // clear_freed_nodes() routine. Fragmentation is not a problem because we
    // allocate nodes from a pool allocator.
    //
    bool Freed;

    // See note above about not getting the right info from the extern "C"
    // versions and C++ operators new/delete.
    // @TODO Remove this and instead save a call stack (optional!, that's a lot
    // of info).
    source_location FreedAt;

    //
    // When allocating a block the caller can mark it as a leak (using the LEAK
    // allocation flag). See :AllocationFlags:
    //
    // That means that it doesn't get freed before the end of the program (since
    // the OS claims back the memory anyway). When
    // Context->DebugMemoryPrintListOfUnfreedAllocationsAtThreadExitOrProgramTermination
    // is set to true we log a list of unfreed allocations but you can also call
    // debug_memory_report_leaks() at any time.
    //
    // Blocks which have this flag don't get reported (they are "meant" to
    // leak).
    //
    bool MarkedAsLeak;
...
// and check memory integrity (detect buffer under/overruns, modifying freed
// memory, freeing the same pointer twice, etc.).
//
// The list is doubly-linked and sorted by the value of the pointer of the
// allocation (in increasing order).
//
// We also detect if allocator implementations return overlapping blocks,
// which may happen if two allocators use the same pool, or the implementation
// itself has a bug.
//
//
...
// Assuming that the heap is not corrupted, this reports any unfreed
// allocations. Yes, the OS claims back all the memory the program has
// allocated anyway; we are not promoting C++ style RAII which make EVEN
// program termination slow. We give this information to the programmer
// because they might find that useful.
...
T *arche_reallocate_impl(T *block, s64 newCount, u64 options, source_location loc) {
    ...
    // reallocate?), so we leave that up to the call site.
    assert(newCount != 0);
...
#if COMPILER == MSVC and (defined DEBUG_OPTIMIZED or defined RELEASE) and (defined ARCHE_NO_CRT)
...
#else
#if COMPILER == MSVC
// Note: If you get many compile errors (but you have defined ARCHE_NO_CRT).
...
// The templated ones are in the arche namespace in order to not conflict too
