...
// performance when looking up things. Some tables work by storing linked lists
// of entries, but that can lead to many more cache misses.
//
// When storing a value, we map its hash to a slot index and if that slot is
// free, we put the key and value there, otherwise we keep incrementing the slot
// index until we find an empty slot. Because the hash table can never be full,
// we are guaranteed to find a slot eventually.
//
// When looking up a value we perform the same process to find the correct slot.
//
// We use hash values to indicate whether slots are empty of removed. A hash of
// 0 means that slot is not used, so new values an be put there. A hash of 1
// means that slot used to be valid, but has been removed. A hash of 2 or higher
// (FIRST_VALID_HASH) means this is a currently used slot.
//
// Whether we hash a key, if the result is less than 2, we just add 2 to it to
// put it in the valid range. This means that 4 values of the hash range have
// double probabilty of collisions, but it's a small price to pay.
//
// We can't use keys/values to indicate whether slots are empty or removed,
// because they can be of arbitrary type.
//
// This is a simple implementation, but still fast because we try to keep only
// 70% of the table full at any time. If you are not memory constrained, just
// make the table bigger and it will get faster. Most of the queries are
// satisfied in the first found slot. The hashing function is also very
// important for this.
//
// Because of that, storing entries compactly (hash, key, value) is best,
// because you always take exactly one cache miss per query. Using multiple
...
template <typename T>
bool compare_equals_default(T const& a, T const& b) {
...
template <typename T>
using equal_to = bool (*)(T const& a, T const& b);
...
struct hash_table {
    ...
    hash_table(initializer_list<pair<K, V>> init) {
        for (const auto &p : init) { add(*this, p.first, p.second); }
    }
...
// _MINIMUM_SIZE_.
//
// Allocates a buffer if the hash table doesn't already point to allocated
// memory (using the Context's allocator).
//
// You don't need to call this before using the hash table.
// The first time an element is added to the hash table, it reserves with
...
void free(any_hash_table auto & table) {
    free(table.Entries);
    table.Allocated   = 0;
    table.Count       = 0;
    table.SlotsFilled = 0;
...
void reset(any_hash_table auto & table) {
    For(range(table.Allocated)) { (table.Entries.Data + it)->Hash = 0; }
    table.Count       = 0;
    table.SlotsFilled = 0;
}

template <any_hash_table T>
s64 search_prehashed_index(T & table, u64 hash, table_key_t<T> const& key) {
    ...
    For_as(_, range(table.Allocated)) {
        ...
        if (it->Hash == 0) return -1;

        if (it->Hash != 1) {
            if (it->Hash == hash && table.KeysEqual(it->Key, key)) return index;
        }
...
template <any_hash_table T>
key_value_pair<T> search_prehashed(T & table, u64 hash, table_key_t<T> const& key) {
...
template <any_hash_table T>
auto search(T & table, table_key_t<T> const& key, table_search_options options = {}) {
    return search_prehashed(table, get_hash(key), key);
}

// Returns pointers to the added key and value.
template <any_hash_table T>
key_value_pair<T> add_prehashed(T & table, u64 hash, table_key_t<T> const& key, table_value_t<T> const& value) {
    ...
    // first item.
    if ((table.SlotsFilled + 1) * 100 >= table.Allocated * T::LOAD_FACTOR_PERCENT) resize(table, table.SlotsFilled * 2);  // Double size
...
template <any_hash_table T>
key_value_pair<T> add(T & table, table_key_t<T> const& key, table_value_t<T> const& value) {
    return add_prehashed(table, get_hash(key), key, value);
}

template <any_hash_table T>
key_value_pair<T> set_prehashed(T & table, u64 hash, table_key_t<T> const& key, table_value_t<T> const& value) {
...
template <any_hash_table T>
key_value_pair<T> set(T & table, table_key_t<T> const& key, table_value_t<T> const& value) {
    return set_prehashed(table, get_hash(key), key, value);
}

template <any_hash_table T>
void remove_prehashed_index(T & table, s64 index) {
    auto *entry  = table.Entries.Data + index;
...
template <any_hash_table T>
bool remove_prehashed(T & table, u64 hash, table_key_t<T> const& key) {
    s64 index = search_prehashed_index(table, hash, key);
...
// Returns true if the key was found and removed.
template <any_hash_table T>
bool remove(T & table, table_key_t<T> const& key) {
    return remove_prehashed(table, get_hash(key), key);
}

// Returns true if the hash table has the given key.
template <any_hash_table T>
bool has(T & table, table_key_t<T> const& key) {
    return search(table, key).Key != null;
}

// Returns true if the hash table has the given key.
template <any_hash_table T>
bool has_prehashed(T & table, u64 hash, table_key_t<T> const& key) {
...
inline u64 get_hash(any_hash_table auto value) { return get_hash(value.Entries); }
