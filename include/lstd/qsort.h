...
void quick_sort(T *first, s64 count, quick_sort_comparison_func<T> compare = default_comparison<T>) {
    auto compareVoid = [&](const void *lhs, const void *rhs) {
        return compare((const T *)lhs, (const T *)rhs);
    };
...
#define INSERTION_SORT_LEFT \
    for (index = left + size; index < store; index += size) INSERTION_SORT_LOOP(left)

#define INSERTION_SORT_RIGHT \
    for (index = store + (size << 1); index <= right; index += size) INSERTION_SORT_LOOP(store + size)
