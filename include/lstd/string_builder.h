            ...
            if (p < end) {
                ...
                if (sz > 0 && p + sz <= end && utf8_is_valid_cp(p)) {
                    ...
                    if (next_cc != 0) {
                        ...
                        if (p - ascii_start > 1) { add(builder, ascii_start, (s64)(p - ascii_start - 1)); }
...
inline bool utf8_normalize_nfd_to_string_builder(const char *str, s64 byteLength, any_string_builder auto ref builder) {
    ...
    while (p < end) {
        ...
        for (s64 i = 0; i < segN; ++i) { add(builder, segBuf.Data[i]); }
    }
...
struct string_builder_writer : writer {
    ...
    void write(const char *data, s64 count) override { add(*Builder, data, count); }
