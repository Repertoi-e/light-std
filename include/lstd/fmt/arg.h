...
// Holds a type and a value. If the value is not arithmetic (custom or string
// type) then the life time of the parameter isn't extended (we just hold a
// pointer)! That means that the parameters need to outlive the parse and format
// function itself.
struct fmt_arg {
...
template <typename T>
auto fmt_mapped_type_constant_v =
    fmt_type_constant_v<decltype(fmt_map_arg(declval<T>()))>;

fmt_arg fmt_make_arg(auto no_copy v) {
  return {fmt_mapped_type_constant_v<decltype(v)>, fmt_value(fmt_map_arg(v))};
}
