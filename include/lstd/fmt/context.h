...
struct fmt_context : writer {
  ...
  fmt_interp
      Parse; // Holds the format string (and how much we've parsed) and some
...
struct fmt_context_visitor {
  ...
  void operator()(fmt_custom_value custom) {
...
// We format signed integers by writing "-" if the integer is negative and then
// calling this routine as if the integer is positive. Note: This is not
// exported, instead use one of the overloads of the general write() function.
void write_u64(fmt_context *f, u64 value, bool negative, fmt_specs specs);

// Writes a float with given formatting specs.
// Note: This is not exported, instead use one of the overloads of the general
// write() function.
...
inline char FORMAT_UINT_DIGITS[] = "0001020304050607080910111213141516171819"
...
inline void write_helper(fmt_context *f, const char *data, s64 size) {
  ...
  if (f->Specs->Type) {
    ...
    if (f->Specs->Type != 's') {
      on_error(f, "Invalid type specifier for a string",
...
inline void write(fmt_context *f, const void *value) {
  if (f->Specs && f->Specs->Type && f->Specs->Type != 'p') {
    on_error(f, "Invalid type specifier for a pointer",
             f->Parse.It.Data - f->Parse.FormatString.Data - 1);
    return;
  }
...
inline void write_u64(fmt_context *f, u64 value, bool negative,
  ...
  if (type == 'd' || type == 'n') {
    ...
    on_error(f, "Invalid type specifier for an integer",
             f->Parse.It.Data - f->Parse.FormatString.Data - 1);
    return;
  }
...
// arabic numerals so ...
//
inline void write_significand(fmt_context *f, string significand,
                              s64 integralSize, code_point decimalPoint = 0) {
...
inline void write_float_fixed(fmt_context *f, string significand, s32 exp,
                              ...
                              bool percentage) {
  s64 outputSize = (sign ? 1 : 0) + (percentage ? 1 : 0) +
...
inline void write_float(fmt_context *f, is_floating_point auto value,
  ...
  s32 exp = fmt_format_non_negative_float(&floatBuffer, value, specs.Precision,
                                          floatSpecs);
