...
//
// This is a module which provides routines to parse integers and booleans.
// As well as helper functions to treat parse status and expecting a sequence of
// characters.
//
// @TODO: Floating point numbers, however we have implementations for strtod,
// atof
...
//
// Returns:
//   * PARSE_SUCCESS          if a valid integer was parsed. A valid integer is
//   in the form (+|-)[digit]*
//                            (where digit may be a letter depending on the base
//                            and Options.CodePointToDigit).
//
//   * PARSE_INVALID          if the function wasn't able to parse a valid
//   integer.
//                            Note: If the integer starts with +/-, that could
//                            be considered invalid (depending on
//                            Options.ParseSign).
//
//   * PARSE_TOO_MANY_DIGITS  if the parsing stopped because the integer became
//   too large (only if Options.BailOnTooManyDigits and
//                            we aren't parsing a big_integer which handles
//                            practically infinite digits). In that case the max
//                            value of the integer type is returned (or min
//                            value if parsing a negative number).
//
...
//
// There are 3 return values: the value parsed, the parse status and the rest of
// the buffer after consuming some characters for the parsing. If the status was
// PARSE_INVALID then some bytes could have been consumed. E.g. when parsing
// words and the buffer was "truDD" _rest_ is "DD".
//
// Allows for compilation of different code paths using a template parameter
// which is a struct literal (parse_bool_options).
//
// This function doesn't eat white space from the beginning.
//
// Returns:
//   * PARSE_SUCCESS          if a valid bool was parsed (0/1 or "true"/"false",
//   depending on the options)
//   * PARSE_EXHAUSTED        if an empty buffer was passed or we started
//   parsing a word but ran out of bytes
