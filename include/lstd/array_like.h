...
//  * reserve, maybe_grow, insert_at_index, insert_at_index, add, add,
//  remove_ordered_at_index, remove_unordered_at_index, remove_ordered,
//  remove_unordered, remove_range, replace_range, remove_all, replace_all
//
// ... for structures that have members Data and Count (and Allocated for
// dynamic arrays)
// - we call these array-likes.
//
// Your custom types (which aren't explicitly flagged) will also automatically
// get this treatment. You can explicitly disable this with a member "static
// const bool TREAT_AS_ARRAY_LIKE = false;"
//
// e.g. stack_array, array, array_view, string - all have the members Data and
// Count, so functions are automatically generated to all these types using the
// definition below. They also resolve for different (but compatible) types
// (e.g. comparing stack_array<u8> and array<u8>) for which otherwise we need a
// combinatorial amount of code to support.
//
// @TODO: We currently expect the members to be named exactly Data and Count
// which may conflict with other naming styles. How should we handle this?
//
...
concept is_array_like =
    should_array_be_treated_as_array_like<T>() && has_array_members<T>;

template <typename T>
concept is_dynamic_array_like =
    should_array_be_treated_as_array_like<T>() && has_dynamic_array_members<T>;
}  // namespace internal

// True if the type has _Data_ and _Count_ members (and the optional explicit
// "TREAT_AS_ARRAY_LIKE" flag is not false).
template <typename T>
concept any_array_like = internal::is_array_like<remove_cv_t<T>>;

// True if the type has _Data_, _Count_, and _Allocated_ members (and the
// optional explicit "TREAT_AS_ARRAY_LIKE" flag is not false).
template <typename T>
concept any_dynamic_array_like =
    internal::is_dynamic_array_like<remove_cv_t<T>>;
...
// and +1 otherwise.
s32 compare_lexicographically(any_array_like auto no_copy a,
                              any_array_like auto no_copy b);

bool operator==(any_array_like auto no_copy a, any_array_like auto no_copy b) {
  return compare(a, b) == -1;
}
bool operator!=(any_array_like auto no_copy a, any_array_like auto no_copy b) {
  return compare(a, b) != -1;
}
auto operator<=>(any_array_like auto no_copy a, any_array_like auto no_copy b) {
...
//
// When modifying dynamic arrays, the object grows and allocates
// automatically, so you can skip managing this, but coming up with a good
...
auto *insert_at_index(
    Arr ref arr, s64 index,
    array_data_t<Arr> no_copy
        element);  // Returns pointer in the array to the added element
...
requires(is_same<array_data_t<Arr>, array_data_t<Arr2>>) auto *insert_at_index(
    Arr ref arr, s64 index, Arr2 no_copy arr2) {
  return insert_at_index(arr, index, arr2.Data, arr2.Count);
}  // Returns pointer in the array to the beginning of added elements

template <any_dynamic_array_like Arr>
auto *insert_at_index(any_dynamic_array_like auto ref arr, s64 index,
                      initializer_list<array_data_t<Arr>> list) {
...
template <any_dynamic_array_like Arr, any_array_like Arr2>
requires(is_same<array_data_t<Arr>, array_data_t<Arr2>>) auto *add(
    Arr ref arr, Arr2 no_copy arr2) {
  return insert_at_index(arr, arr.Count, arr2);
}

template <any_dynamic_array_like Arr>
auto *add(Arr ref arr, initializer_list<array_data_t<Arr>> list) {
  return insert_at_index(arr, arr.Count, list.begin(),
                         list.end() - list.begin());
}
...
template <any_dynamic_array_like Arr, any_array_like Arr2>
requires(is_same<array_data_t<Arr>, array_data_t<Arr2>>) auto &operator+=(
    Arr ref arr, Arr2 no_copy arr2) {
  add(arr, arr2);
  return arr;
}
...
// moves following elements forward/backward if (end - begin) !=
// replace.Count.
template <any_dynamic_array_like Arr, any_array_like Arr2>
...
template <any_dynamic_array_like Arr, any_array_like Arr2>
requires(is_same<array_data_t<Arr>, array_data_t<Arr2>>) void remove_all(
    Arr ref arr, Arr2 no_copy search) {
...
s64 search(Arr no_copy arr, delegate<bool(array_data_t<Arr> no_copy)> predicate,
  ...
  For(range(options.Start, options.Reversed ? -1 : arr.Count,
            options.Reversed ? -1 : 1)) if (predicate(arr.Data[it])) return it;
  return -1;
}
...
requires(is_same<array_data_t<Arr>, array_data_t<Arr2>> &&is_same<
  ...
  if (search.Count == replace.Count) {
    ...
    s64 i = 0;
    while (i < arr.Count &&
           (i = ::search(arr, search, search_options{.Start = i})) != -1) {
      replace_range(arr, i, i + search.Count,
