...
inline void lock(mutex *m) { EnterCriticalSection((CRITICAL_SECTION *)m->PlatformData.Win32.Handle); }

inline bool try_lock(mutex *m) { return TryEnterCriticalSection((CRITICAL_SECTION *)m->PlatformData.Win32.Handle); }

inline void unlock(mutex *m) { LeaveCriticalSection((CRITICAL_SECTION *)m->PlatformData.Win32.Handle); }
...
inline void terminate(thread t) {
    if (t.Handle) { TerminateThread(t.Handle, 0); }
}
