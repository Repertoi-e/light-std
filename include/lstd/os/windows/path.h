...
//
// This module provides facilities to work with paths and files, WindowsNT/95
// version. Also provides facilities to query the OS and move/copy/read/write,
// etc. All functions are prefixed with "path_" so you can find them easily with
// autocomplete.
//
// We import this module when compiling for Windows.
// If you want to explicitly work with Windows paths, import this module
// directly.
//
...
// This function is only exported for NT. POSIX paths don't have drives/UNC
// share points.
//
...
private:
  ...
  // I don't usually use "private" but we seriously don't need to expose this
  // garbage to users...
  friend void path_read_next_entry(path_walker ref walker);
};

// @Cleanup
inline void free_path_walker(path_walker ref walker) {
...
inline mark_as_leak string path_join(string one, string other) {
  auto arr = make_stack_array(one, other);
  return path_join(arr);
}

inline mark_as_leak string path_normalize(string path) {
  string result;
