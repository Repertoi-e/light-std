...
inline void setup_console() {
    // Retrieve terminal file descriptors
    int cinHandle = STDIN_FILENO;
    int coutHandle = STDOUT_FILENO;
    int cerrHandle = STDERR_FILENO;
    ...
    struct termios term;
    if (tcgetattr(coutHandle, &term) != -1) {
        term.c_lflag |= ECHOCTL | ECHOKE;
        if (tcsetattr(coutHandle, TCSAFLUSH, &term) == -1) {
