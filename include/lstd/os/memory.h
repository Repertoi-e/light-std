...
inline void platform_report_warning(string message, source_location loc = source_location::current()) {
    print(">>> {!YELLOW}Platform warning{!} {}:{} (in function: {}): {}.\n", loc.file_name(), loc.line(), loc.function_name(), message);
}

inline void platform_report_error(string message, source_location loc) {
    print(">>> {!RED}Platform error{!} {}:{} (in function: {}): {}.\n", loc.file_name(), loc.line(), loc.function_name(), message);
}

inline void create_new_temp_storage_block(s64 size) {
    if (S->TempAllocData.Block) { os_free_block(S->TempAllocData.Block); }
...
inline void *platform_temp_alloc(allocator_mode mode, void *context, s64 size, void *oldMemory, s64 oldSize, u64 options) {
...
inline void *create_persistent_alloc_page(s64 size) {
    void *result = os_allocate_block(size + sizeof(platform_memory_state::persistent_alloc_page));

    auto *p = (platform_memory_state::persistent_alloc_page *)result;

    p->Next = S->PersistentAllocBasePage;
    if (!S->PersistentAllocBasePage) { S->PersistentAllocBasePage = p; }

    return (void *)(p + 1);
}

inline void *platform_persistent_alloc(allocator_mode mode, void *context, s64 size, void *oldMemory, s64 oldSize, u64 options);
