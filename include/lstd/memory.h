...
export {
  ...
  using allocator_func_t =
      void *(*)(allocator_mode mode, void *context, s64 size, void *oldMemory,
                s64 oldSize, u64 options);
  ...
  struct allocator {
    ...
    allocator(allocator_func_t function, void *context)
  ...
  template <non_void T>
  T *lstd_allocate_impl(s64 count, allocator alloc, u32 alignment, u64 options,
                        source_location loc);
  ...
  T *malloc(allocate_options options = {},
    ...
    return lstd_allocate_impl<T>(options.Count, options.Alloc,
                                 options.Alignment, options.Options, loc);
  }
  ...
  template <non_void T>
  T *realloc(T * block, reallocate_options options,
             source_location loc = source_location::current()) {
    return lstd_reallocate_impl<T>(block, options.NewCount, options.Options,
                                   loc);
  }
  ...
  void tlsf_allocator_add_pool(tlsf_allocator_data * data, void *block,
                               s64 size) {
    if (!data->State) {
