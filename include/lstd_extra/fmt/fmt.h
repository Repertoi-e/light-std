...
// There is a quirk with modules name mangling which means we have to use a
// header file just for this function to be visible everywhere and be able to be
// specialized.
...
template <typename T> void write_custom(fmt_context *f, const T *t) {
  // XXX TEMP static_assert(false, "Argument doesn't have a way to be
  // formatted.");
  assert(false);
