...
// callback later
template <typename F>
s64 connect(delegate_signal<F> & s, delegate<F> cb) {
...
template <typename F>
bool disconnect(delegate_signal<F> & s, s64 index) {
...
template <typename T>
void handle_to_remove_after_emit_end(delegate_signal<T> & s) {
    For(s.ToRemove) {
...
template <typename R, typename... Args>
void emit(delegate_signal<R(Args...)> & s, Args const&... args) {
    s.CurrentlyEmitting = true;
    For(s.Callbacks) if (it) it((Args const&)args...);
...
template <typename R, typename... Args>
void emit_while_false(delegate_signal<R(Args...)> & s, Args... args) {
    static_assert(is_convertible<R, bool>);

    s.CurrentlyEmitting = true;
    For(s.Callbacks) if (it) if (it((Args const&)args...)) break;
    s.CurrentlyEmitting = false;
    handle_to_remove_after_emit_end(s);
}
...
template <typename R, typename... Args>
void emit_while_true(delegate_signal<R(Args...)> & s, Args... args) {
    static_assert(is_convertible<R, bool>);
