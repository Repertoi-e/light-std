...
struct default_ieee754_traits {
    ...
    static T carrier_to_float(carrier_uint u) noexcept {
        T x;
        LSTD_NAMESPACE::copy_memory(&x, &u, sizeof(carrier_uint));
        return x;
    }
    static carrier_uint float_to_carrier(T x) noexcept {
        carrier_uint u;
        LSTD_NAMESPACE::copy_memory(&u, &x, sizeof(carrier_uint));
        return u;
    }
    ...
    static constexpr bool is_negative_infinity(carrier_uint u) noexcept {
        ...
        constexpr auto negative_infinity =
            (carrier_uint((carrier_uint(1) << exponent_bits) - 1) << significand_bits) | (carrier_uint(1) << (carrier_bits - 1));
...
struct ieee754_bits {
    ...
    constexpr explicit ieee754_bits(carrier_uint bit_pattern) noexcept : u{bit_pattern} {}
    constexpr explicit ieee754_bits(T float_value) noexcept : u{ieee754_traits<T>::float_to_carrier(float_value)} {}
...
struct uint128 {
    ...
    constexpr uint128(std::uint64_t high, std::uint64_t low) noexcept : internal_{((unsigned __int128) low) | (((unsigned __int128) high) << 64)} {}
...
constexpr std::int32_t floor_shift(
    ...
    return shift_amount == 0 ? std::int32_t(integer_part) : std::int32_t((integer_part << shift_amount) | (fractional_digits >> (64 - shift_amount)));
}
...
constexpr bool check_divisibility_and_divide_by_pow5(std::uint32_t &n) noexcept {
    ...
    constexpr std::uint32_t comparison_mask =
        info::bits_for_comparison >= 32 ? std::numeric_limits<std::uint32_t>::max() : std::uint32_t((std::uint32_t(1) << info::bits_for_comparison) - 1);
...
constexpr std::uint32_t small_division_by_pow10(std::uint32_t n) noexcept {
    ...
    return (n * small_division_by_pow10_info<N>::magic_number) >> small_division_by_pow10_info<N>::shift_amount;
}
...
struct compressed_cache_detail {
    ...
    static constexpr std::size_t compressed_table_size =
        (cache_holder<ieee754_format::binary64>::max_k -
         cache_holder<ieee754_format::binary64>::min_k + compression_ratio) /
        compression_ratio;
    ...
    static constexpr std::uint32_t errors[] = {
        0x50001400, 0x54044100, 0x54014555, 0x55954415, 0x54115555, 0x00000001, 0x50000000, 0x00104000, 0x54010004, 0x05004001, 0x55555544, 0x41545555, 0x54040551, 0x15445545, 0x51555514, 0x10000015, 0x00101100, 0x01100015, 0x00000000, 0x00000000, 0x00000000, 0x00000000, 0x04450514, 0x45414110, 0x55555145, 0x50544050, 0x15040155, 0x11054140, 0x50111514, 0x11451454, 0x00400541, 0x00000000, 0x55555450, 0x10056551, 0x10054011, 0x55551014, 0x69514555, 0x05151109, 0x00155555};
};
}  // namespace detail
...
struct to_even : base {
    ...
    static constexpr void break_rounding_tie(Fp &fp) noexcept {
        fp.significand = fp.significand % 2 == 0 ? fp.significand : fp.significand - 1;
    }
};
        ...
        if constexpr (format == ieee754_format::binary64) {
            ...
            if (offset == 0) {
                ...
                auto error     = (compressed_cache_detail::errors[error_idx] >>
                              ((k - cache_holder<format>::min_k) % 16) * 2) &
                             0x3;
...
struct impl : private ieee754_traits<Float>,
    ...
    static constexpr int max_k = [] {
        ...
        constexpr auto b = -log::floor_log10_pow2(
                               int(min_exponent - significand_bits)) +
                           kappa;
        return a > b ? a : b;
    }();
    ...
    template <class ReturnType, class IntervalTypeProvider, class SignPolicy, class TrailingZeroPolicy, class CorrectRoundingPolicy, class CachePolicy>
        ...
        if (exponent != 0) {
            ...
            if (significand == 0) {
                shorter_interval_case<TrailingZeroPolicy, CorrectRoundingPolicy, CachePolicy>(
                    ret_value, exponent, IntervalTypeProvider::interval_type_shorter(br));
                return ret_value;
            }
    ...
    template <class TrailingZeroPolicy, class CorrectRoundingPolicy, class CachePolicy, class ReturnType, class IntervalType>
    ...
    compute_right_closed_directed(ieee754_bits<Float> const br) noexcept {
        ...
        // Compute zi and deltai
        // 10^kappa <= deltai < 10^(kappa + 1)
        auto const deltai     = closer_boundary ? compute_delta(cache, beta - 2) : compute_delta(cache, beta - 1);
    ...
    JKJ_FORCEINLINE static int remove_trailing_zeros(carrier_uint &n) noexcept {
        ...
        if constexpr (format == ieee754_format::binary32) {
            ...
            n = (remainder >> 7) + quotient * carrier_uint(10);
            return 7;
        }
    }
...
constexpr auto convert_to_policy_holder(
    ...
    return convert_to_policy_holder(found_policy_pair_list<repeated, RemainingFoundPolicyPairs...>{},
                                    typename FirstFoundPolicyPair::policy{},
                                    policies...);
}
    ...
    return policy_holder::delegate(br,
                                   [br](auto interval_type_provider) {
                                       ...
                                       if constexpr (tag == rounding_mode::tag_t::to_nearest) {
                                           return detail::impl<Float>::template compute_nearest<return_type, decltype(interval_type_provider), typename policy_holder::sign_policy, typename policy_holder::trailing_zero_policy, typename policy_holder::correct_rounding_policy, typename policy_holder::cache_policy>(br);
                                       } else if constexpr (tag == rounding_mode::tag_t::left_closed_directed) {
                                           return detail::impl<Float>::template compute_left_closed_directed<return_type,
                                                                                                             typename policy_holder::sign_policy,
                                                                                                             typename policy_holder::trailing_zero_policy,
                                                                                                             typename policy_holder::cache_policy>(br);
                                       } else {
                                           return detail::impl<Float>::template compute_right_closed_directed<return_type,
                                                                                                              typename policy_holder::sign_policy,
                                                                                                              typename policy_holder::trailing_zero_policy,
                                                                                                              typename policy_holder::cache_policy>(br);
                                       }
