...
export {
    ...
    void require_arithmetic_arg(fmt_parse_context * p, fmt_type argType, s64 errorPosition = -1) {
        ...
        if (!fmt_is_type_arithmetic(argType)) p->on_error("Format specifier requires an arithmetic argument", errorPosition);
    }
    ...
    void require_signed_arithmetic_arg(fmt_parse_context * p, fmt_type argType, s64 errorPosition = -1) {
        ...
        if (fmt_is_type_integral(argType) && argType != fmt_type::S64) {
            p->on_error("Format specifier requires a signed integer argument (got unsigned)", errorPosition);
        }
    }
    ...
    void check_precision_for_arg(fmt_parse_context * p, fmt_type argType, s64 errorPosition = -1) {
        ...
        if (fmt_is_type_integral(argType)) {
            p->on_error("Precision is not allowed for integer types", errorPosition);
        }
        if (argType == fmt_type::Pointer) {
            p->on_error("Precision is not allowed for pointer type", errorPosition);
        }
    }

    u32 next_arg_id(fmt_parse_context * p) {
        if (p->NextArgID >= 0) return (u32) p->NextArgID++;
        p->on_error("Cannot switch from manual to automatic argument indexing");
        return 0;
    }

    bool check_arg_id(fmt_parse_context * p, u32) {
        if (p->NextArgID > 0) {
            p->on_error("Cannot switch from automatic to manual argument indexing");
            return false;
        }
        p->NextArgID = -1;
        return true;
    ...
    file_scope fmt_alignment get_alignment_from_char(utf8 ch) {
        if (ch == '<') {
    ...
    bool parse_fill_and_align(fmt_parse_context * p, fmt_type argType, fmt_specs * specs) {
        ...
        if (align != fmt_alignment::NONE) {
            ...
            if (align == fmt_alignment::NUMERIC) require_arithmetic_arg(p, argType, errorPosition - 1);
        }
        return true;
    }
