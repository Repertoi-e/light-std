...
const u64 POW10_SIGNIFICANDS_64[] = {
...
u32 cache_compute_left_endpoint_for_shorter_interval_case(u64 cache, s32 beta_minus_1) {
    return (u32)((cache - (cache >> (F32_MANT_BITS + 2))) >> (64 - F32_MANT_BITS - 1 - beta_minus_1));
}

u32 cache_compute_right_endpoint_for_shorter_interval_case(u64 cache, s32 beta_minus_1) {
    return (u32)((cache + (cache >> (F32_MANT_BITS + 1))) >> (64 - F32_MANT_BITS - 1 - beta_minus_1));
}

u32 cache_compute_round_up_for_shorter_interval_case(u64 cache, s32 beta_minus_1) { return ((u32)(cache >> (64 - F32_MANT_BITS - 2 - beta_minus_1)) + 1) / 2; }

u64 compute_mul(u64 u, u128 cache) { return umul192_upper64(u, cache); }

u32 compute_delta(u128 cache, s32 beta_minus_1) { return (u32)(cache.hi >> (64 - 1 - beta_minus_1)); }

bool compute_mul_parity(u64 two_f, u128 cache, s32 beta_minus_1) {
    assert(beta_minus_1 >= 1);
    assert(beta_minus_1 < 64);
...
u64 compute_left_endpoint_for_shorter_interval_case(u128 cache, s32 beta_minus_1) {
    return (cache.hi - (cache.hi >> (F64_MANT_BITS + 2))) >> (64 - F64_MANT_BITS - 1 - beta_minus_1);
}

u64 compute_right_endpoint_for_shorter_interval_case(u128 cache, s32 beta_minus_1) {
    return (cache.hi + (cache.hi >> (F64_MANT_BITS + 1))) >> (64 - F64_MANT_BITS - 1 - beta_minus_1);
}

u64 compute_round_up_for_shorter_interval_case(u128 cache, s32 beta_minus_1) {
    return ((cache.hi >> (64 - F64_MANT_BITS - 2 - beta_minus_1)) + 1) / 2;
}
...
always_inline auto shorter_interval_case(s32 exponent) {
    ...
    result.Significand = compute_round_up_for_shorter_interval_case(cache, beta_minus_1);
...
export {
    ...
    auto to_decimal(types::is_floating_point auto x) {
        ...
        // Compute k and beta.
        const s32 minus_k      = floor_log10_pow2(exponent) - KAPPA;
        auto cache             = IS_F32 ? get_cached_power_f32(-minus_k) : get_cached_power_f64(-minus_k);
        const s32 beta_minus_1 = exponent + floor_log2_pow10(-minus_k);

        // Compute zi and deltai
        // 10^kappa <= deltai < 10^(kappa + 1)
        const u32 deltai    = compute_delta(cache, beta_minus_1);
        const uint_t two_fc = significand << 1;
        const uint_t two_fr = two_fc | 1;
        const uint_t zi     = compute_mul(two_fr << beta_minus_1, cache);
        ...
        if (r > deltai) {
            ...
            if ((!include_left_endpoint || !is_endpoint_integer<IS_F32>(two_fl, exponent, minus_k)) && !compute_mul_parity(two_fl, cache, beta_minus_1)) {
                goto small_divisor_case_label;
            }
        }
    ...
    small_divisor_case_label:
        ...
        auto dist = r - (deltai / 2) + (BIG_DIVISOR / 2);

        // Is dist divisible by 2^kappa?
        if ((dist & mask) == 0) {
            bool approx_y_parity = ((dist ^ (BIG_DIVISOR / 2)) & 1) != 0;
            dist >>= KAPPA;
