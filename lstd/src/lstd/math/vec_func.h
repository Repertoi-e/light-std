...
bool is_null_vector(const vec<T, Dim, Packed> &v) {
    static constexpr T epsilon = T(1) / const_exp10<T>(const_abs(type::numeric_info<T>::min_exponent10) / 2);
    T length = len(v);
...
vec<T, Dim, Packed> safe_normalize(const vec<T, Dim, Packed> &v) {
    ...
    vmod[0] = abs(v[0]) > type::numeric_info<T>::denorm_min() ? v[0] : type::numeric_info<T>::denorm_min();
...
requires (type::is_convertible_v<U, T>) void fill(vec<T, Dim, Packed> &lhs, U all) {
...
vec<T, 3, Packed> cross(const vec<T, 3, Packed> &lhs, const vec<T, 3, Packed> &rhs) {
    ...
    if constexpr (type::has_simd_v<VecT>) {
