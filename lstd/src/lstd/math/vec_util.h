...
auto operator|(const swizzle<VectorData1, Indices1...> &lhs, const swizzle<VectorData2, Indices2...> &rhs) {
    using TS1 = typename vec_info<VectorData1>::type;
    using TS2 = typename vec_info<VectorData2>::type;
...
auto operator|(const swizzle<VectorData1, Indices1...> &lhs, const vec<T2, Dim, Packed> &rhs) {
    using TS = typename vec_info<VectorData1>::type;
...
template <typename T, s64 Dim, bool Packed, typename U>
inline enable_if_t<is_convertible_v<U, T>, vec<T, Dim, Packed> &> operator*=(vec<T, Dim, Packed> &lhs, U rhs) {
    if constexpr (!has_simd_v<vec<T, Dim, Packed>>) {
...
// Scales the vector by _rhs_
template <typename T, s64 Dim, bool Packed, typename U>
inline enable_if_t<is_convertible_v<U, T>, vec<T, Dim, Packed>> operator*(const vec<T, Dim, Packed> &lhs, U rhs) {
    if constexpr (!has_simd_v<vec<T, Dim, Packed>>) {
...
template <typename T, s64 Dim, bool Packed, typename U>
inline enable_if_t<is_convertible_v<U, T>, vec<T, Dim, Packed>> operator*(U lhs, const vec<T, Dim, Packed> &rhs) {
    return rhs * lhs;
}
...
auto operator*(const vec<T, Dim, Packed> &v, const swizzle<VectorDataT, Indices...> &s)
    -> enable_if_t<Dim == sizeof...(Indices) && is_same_v<T, typename vec_info<VectorDataT>::type>,
                   vec<T, Dim, Packed>> {
...
template <typename T, s64 Dim, bool Packed, typename VectorDataT, s64... Indices>
auto operator*(const swizzle<VectorDataT, Indices...> &s, const vec<T, Dim, Packed> &v)
    -> enable_if_t<Dim == sizeof...(Indices) && is_same_v<T, typename vec_info<VectorDataT>::type>,
                   vec<T, Dim, Packed>> {
...
template <typename T, s64 Dim, bool Packed, typename VectorDataT, s64... Indices>
auto operator*=(vec<T, Dim, Packed> &v, const swizzle<VectorDataT, Indices...> &s)
    -> enable_if_t<Dim == sizeof...(Indices) && is_same_v<T, typename vec_info<VectorDataT>::type>, vec<T, Dim, Packed>>
        & {
    return v *= decltype(v)(s);
}
...
template <typename T, s64 Dim, bool Packed, typename VectorDataT, s64... Indices>
auto operator*=(swizzle<VectorDataT, Indices...> &s, const vec<T, Dim, Packed> &v)
    -> enable_if_t<Dim == sizeof...(Indices) && is_same_v<T, typename vec_info<VectorDataT>::type>,
                   swizzle<VectorDataT, Indices...>> & {
...
auto operator*(const swizzle<VData1, Indices1...> &s1, const swizzle<VData2, Indices2...> &s2) {
    using V1 = vec<typename vec_info<VData1>::type, vec_info<VData1>::Dim, vec_info<VData1>::Packed>;
    using V2 = vec<typename vec_info<VData2>::type, vec_info<VData2>::Dim, vec_info<VData2>::Packed>;
    return V1(s1) * V2(s2);
}
...
template <typename VectorDataT, s64... Indices, typename U,
          typename = enable_if_t<is_convertible_v<U, typename vec_info<VectorDataT>::type>>>
auto operator*(const swizzle<VectorDataT, Indices...> &lhs, U rhs) {
    using VectorT =
        vec<typename vec_info<VectorDataT>::type, vec_info<VectorDataT>::Dim, vec_info<VectorDataT>::Packed>;
...
template <typename T, typename U, typename = enable_if_t<!is_vec_v<T> && !is_mat_v<T> && !is_quat_v<T>>>
bool almost_equal(T d1, U d2) {
    using P = mat_mul_elem_t<T, U>;
    return almost_equal(P(d1), P(d2), integral_constant<bool, is_floating_point_v<P>>());
}
