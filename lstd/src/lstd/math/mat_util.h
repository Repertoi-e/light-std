...
auto operator*(const mat<T, R, C, Packed> &lhs, const mat<T2, R, C, Packed> &rhs) {
...
mat<T1, Dim, Dim, Packed> &operator*=(mat<T1, Dim, Dim, Packed> &lhs, const mat<T2, Dim, Dim, Packed> &rhs) {
    lhs = lhs * rhs;
    return lhs;
}
...
auto small_add(const mat<T, R, C, Packed> &lhs, const mat<U, R, C, Packed> &rhs, integer_sequence<StripeIndices...>) {
...
template <typename T, typename U, s64 R, s64 C, bool Packed, s64... StripeIndices>
auto small_sub(const mat<T, R, C, Packed> &lhs, const mat<U, R, C, Packed> &rhs, integer_sequence<StripeIndices...>) {
    using V = mat_mul_elem_t<T, U>;
    using ResultT = mat<V, R, C, Packed>;
    return ResultT{ResultT::FromStripes, lhs.Stripes[StripeIndices] - rhs.Stripes[StripeIndices]...};
}
...
auto operator+(const mat<T, R, C, Packed> &lhs, const mat<U, R, C, Packed> &rhs) {
    using V = mat_mul_elem_t<T, U>;
...
auto operator+(const Mat &lhs, U rhs) { return lhs + Mat(rhs); }

template <any_mat Mat, typename U>
auto operator+(U lhs, const Mat &rhs) { return Mat(lhs) + rhs; }
...
auto operator-(const mat<T, R, C, Packed> &lhs, const mat<U, R, C, Packed> &rhs) {
    ...
    if constexpr (R * C == 4) {
        mat<V, R, C, Packed> result;
        For_as(i, range(R))
            For_as(j, range(C)) result(i, j) = lhs(i, j) - rhs(i, j);
        return result;
    } else if constexpr (R <= 4 && C <= 4) {
...
template <typename T, typename U, s64 R, s64 C, bool Packed>
mat<U, R, C, Packed> &operator+=(mat<T, R, C, Packed> &lhs, const mat<U, R, C, Packed> &rhs) {
    lhs = lhs + rhs;
    return lhs;
}

template <any_mat Mat, typename U>
auto operator+=(Mat &lhs, U rhs) { return lhs += Mat(rhs); }
...
mat<T, R, C, Packed> &operator*=(mat<T, R, C, Packed> &m, U s) {
...
template <typename T, s64 R, s64 C, bool Packed, typename U>
    requires(types::is_convertible<U, T>)
mat<T, R, C, Packed> operator*(const mat<T, R, C, Packed> &m, U s) {
...
template <typename T, s64 R, s64 C, bool Packed, typename U>
    requires(types::is_convertible<U, T>)
mat<T, R, C, Packed> operator*(U s, const mat<T, R, C, Packed> &m) { return m * s; }

template <typename T, s64 R, s64 C, bool Packed, typename U>
    requires(types::is_convertible<U, T>)
mat<T, R, C, Packed> operator/(U s, const mat<T, R, C, Packed> &m) {
    mat<T, R, C, Packed> result;
...
template <typename MatrixDestT, typename MatrixSourceT>
    requires(impl::reinterpret_compatible<MatrixDestT, MatrixSourceT>::value)
MatrixDestT mat_reinterpret_cast(const MatrixSourceT &source) {
    MatrixDestT dest;
