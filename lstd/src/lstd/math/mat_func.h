...
inline auto small_product_row_rr(const mat<T, R1, Match, Packed> &lhs, const mat<U, Match, C2, Packed> &rhs, s64 row, types::integer_sequence<s64, MatchIndices...>) {
...
inline auto small_product_rr(const mat<T, R1, Match, Packed> &lhs, const mat<U, Match, C2, Packed> &rhs, types::integer_sequence<s64, RowIndices...>) {
    using V = types::mat_mul_elem_t<T, U>;
    using ResultT = mat<V, R1, C2, Packed>;
    return ResultT{ResultT::FromStripes, small_product_row_rr(lhs, rhs, RowIndices, types::make_integer_sequence<s64, Match>{})...};
}
}  // namespace impl
...
inline mat<T, R1, C2, Packed> dot(const mat<T, R1, Match, Packed> &lhs, const mat<U, Match, C2, Packed> &rhs) {
    if constexpr (R1 == 4 && Match == 4 && C2 == 4 && types::is_same_v<T, f32> && types::is_same_v<U, f32>) {
        using V = types::mat_mul_elem_t<T, U>;
        ...
        if constexpr (R1 <= 4 && Match <= 4 && C2 <= 4) {
            return impl::small_product_rr(lhs, rhs, types::make_integer_sequence<s64, R1>{});
        } else {
            using V = types::mat_mul_elem_t<T, U>;
...
inline auto dot(const vec<Vt, Vd, Packed> &v, const mat<Mt, Vd, Mcol, Packed> &mat) {
    using Rt = types::mat_mul_elem_t<Vt, Mt>;
