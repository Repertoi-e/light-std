...
struct line {
    ...
    line(const VectorT &p1, const VectorT &p2)
        : line(p1, safe_normalize(p2 - p1)) {
    }
...
struct line_segment {
    ...
    line_segment(const VectorT &p1, const VectorT &p2)
        : P1(p1),
          P2(p2) {
    }
...
public:
    ...
    Triangle3D(const vec<T, 3, false> &a, const vec<T, 3, false> &b, const vec<T, 3, false> &c)
        : a(a),
          b(b),
          c(c) {
    }
