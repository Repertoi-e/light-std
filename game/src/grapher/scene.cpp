...
void update_and_render_scene() {
    ...
    if (GameMemory->MainWindow->is_visible()) {
        ...
        if (Scene->GridFollowCamera) {
            ...
            grid->Position.x = (f32)(s64)(vp->Position.x / s) * s + s / 2;
            grid->Position.y = (f32)(s64)(vp->Position.y / s) * s + s / 2;
        }

        auto cameraOrientation = quat::ROTATION_Y(-vp->Rotation);
        Scene->Uniforms.ViewMatrix = mat4::IDENTITY();
        Scene->Uniforms.ViewMatrix *= mat4::SCALE(vp->Scale);
        Scene->Uniforms.ViewMatrix *= mat4::ROTATE(cameraOrientation.conjugate()) * mat4::TRANSLATE(-vp->Position);
