...
#include <cstdlib>  // @DependencyCleanup
...
static bool SequencerAddDelButton(ImDrawList *draw_list, ImVec2 pos, bool add = true) {
    ...
    if (add) draw_list->AddLine(ImVec2(midx, delRect.Min.y + 3), ImVec2(midx, delRect.Max.y - 3), delColor, 2);
    return overDel;
}

static int min(int a, int b) { return (a < b) ? a : b; }  // @DependencyCleanup
static int max(int a, int b) { return (a > b) ? a : b; }  // @DependencyCleanup
...
bool Sequencer(SequenceInterface *sequence, int *currentFrame, bool *expanded, int *selectedEntry, int *firstFrame,
    ...
    if (visibleFrameCount >= frameCount && firstFrame) *firstFrame = sequence->GetFrameMin();
    ...
    if (expanded && !*expanded) {
        ...
        */
        ...
        if (MovingCurrentFrame) {
            if (frameCount) {
                *currentFrame = (int) ((io.MousePos.x - topRect.Min.x) / framePixelWidth) + firstFrameUsed;
                if (*currentFrame < sequence->GetFrameMin()) *currentFrame = sequence->GetFrameMin();
                if (*currentFrame >= sequence->GetFrameMax()) *currentFrame = sequence->GetFrameMax();
            }
        ...
        auto drawLine = [&](int i, int regionHeight) {
            ...
            int px =
                (int) canvas_pos.x + int(i * framePixelWidth) + legendWidth - int(firstFrameUsed * framePixelWidth);
        ...
        for (int i = 0; i < sequenceCount; i++) {
                ...
                (sequenceOptions & SEQUENCER_EDIT_STARTEND))  // TODOFOCUS && backgroundRect.Contains(io.MousePos))
            {
                ...
                for (int j = 0; j < 3; j++) {
                    ImRect &rc = rects[j];
                    if (!rc.Contains(io.MousePos)) continue;
                    if (!ImRect(childFramePos, childFramePos + childFrameSize).Contains(io.MousePos)) continue;
            ...
            if (localCustomHeight > 0) {
                ...
                ImRect legendClippingRect(
                    ...
                    canvas_pos + ImVec2(float(legendWidth), float(localCustomHeight + ItemHeight)));
    ...
    if (expanded) {
        ...
        if (overExpanded && io.MouseReleased[0]) *expanded = !*expanded;
    }
