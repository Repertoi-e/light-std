...
STBM__API void *stbm_alloc_align(stbm_tc *tc, stbm_heap *heap, size_t size, unsigned short userdata16, size_t alignment,
                                 size_t align_offset);
// allocate a block of memory of 'size' bytes from the heap 'heap'
...
STBM__API void *stbm_alloc_debug(stbm_tc *tc, stbm_heap *heap, size_t size, unsigned short userdata16, size_t alignment,
                                 size_t align_offset, char *file, int line, size_t extra_debug_size);
// allocate a block of memory of 'size' bytes from the heap 'heap';
// userdata16 is an arbitrary 16-bit number stored in the allocation.
...
STBM__API void *stbm_alloc_fileline(stbm_tc *tc, stbm_heap *heap, size_t size, unsigned short userdata16, char *file,
                                    int line);
// allocate a block of memory of 'size' bytes from the heap 'heap';
// userdata16 is an arbitrary 16-bit number stored in the allocation.
//
...
STBM__API void *stbm_alloc_align_fileline(stbm_tc *tc, stbm_heap *heap, size_t size, unsigned short userdata16,
                                          size_t alignment, size_t align_offset, char *file, int line);
// allocate a block of memory of 'size' bytes from the heap 'heap'
// which is aligned to a multiple of 'alignment' (which must be a
// power of two).
//
...
#define STBM_HEAP_SIZEOF                                                                                              \
    (/* alignment     */ 60 /* ct_free_stack */ + sizeof(void *) * 16 + 64 /* ct_free_list  */ +                      \
     64 /* alloc_lock    */ + sizeof(STBM_MUTEX_HANDLE) /* configuration */ + 3 * sizeof(size_t) +                    \
     6 * 4 /* system_alloc  */ + 5 * sizeof(void *) + 3 * sizeof(void *) /* stats         */ + 2 * 4 +                \
     4 * sizeof(void *) /* small         */ + 2 * (6 * sizeof(void *) + 2 * 4) + sizeof(void *) /* medium        */ + \
     3 * 4 + 13 * 4 + 13 * 32 * sizeof(void *) + 4 * sizeof(void *))
...
static void STBM_FAIL(char *message) { STBM_ASSERT(0); }
#endif
...
static int stbm__is_pow2(size_t num) { return (num & (num - 1)) == 0; }
...
stbm__uint32 stbm__size_classes[STBM__NUM_INDICES][STBM__NUM_SLOTS] = {
    ...
    {
        65536,  67584,  69632,  71680,  73728,  75776,  77824,  79872,  81920,  83968,  86016,
        88064,  90112,  92160,  94208,  96256,  98304,  100352, 102400, 104448, 106496, 108544,
        110592, 112640, 114688, 116736, 118784, 120832, 122880, 124928, 126976, 129024,
    },
    {
        131072, 135168, 139264, 143360, 147456, 151552, 155648, 159744, 163840, 167936, 172032,
        176128, 180224, 184320, 188416, 192512, 196608, 200704, 204800, 208896, 212992, 217088,
        221184, 225280, 229376, 233472, 237568, 241664, 245760, 249856, 253952, 258048,
    },
    {
        262144, 270336, 278528, 286720, 294912, 303104, 311296, 319488, 327680, 335872, 344064,
        352256, 360448, 368640, 376832, 385024, 393216, 401408, 409600, 417792, 425984, 434176,
        442368, 450560, 458752, 466944, 475136, 483328, 491520, 499712, 507904, 516096,
    },
    {
        ...
        884736, 901120, 917504, 933888, 950272, 966656, 983040, 999424, 1015808, 1032192,
    },
};
...
static void stbm__stats_update_alloc(stbm_heap *heap, size_t bytes) {
    ...
    if (heap->full_stats) {
        heap->max_outstanding_allocations_count =
            STBM__MAX(heap->max_outstanding_allocations_count, heap->cur_outstanding_allocations_count);
        heap->max_outstanding_allocations_bytes =
            STBM__MAX(heap->max_outstanding_allocations_bytes, heap->cur_outstanding_allocations_bytes);
...
static void *stbm__heap_alloc(stbm_heap *heap, size_t size) {
    if (size <= STBM__SMALLEST_ALLOCATION) return stbm__small_alloc(heap, size);
...
static void stbm__small_free(stbm_heap *heap, void *ptr) {
    ...
    if (chunk_to_free) stbm__medium_free(heap, chunk_to_free);
}
...
static void *stbm__small_alloc(stbm_heap *heap, size_t size) {
    ...
    if (c == &heap->small.nonfull) {
        if (heap->small.chunkcache) {
            ...
            c->unused_storage = (char *) stbm__align_address((char *) c + heap->smallchunk_size - STBM__SMALL_ALIGNMENT,
                                                             STBM__SMALL_ALIGNMENT,
                                                             STBM__SMALL_ALIGNMENT - sizeof(stbm__small_allocated));
    ...
    if (c->first_free) {
        ...
        if (b->u.prefix != STBM__SMALL_FREE_BLOCK_MAGIC) STBM_FAIL("Corrupt data structure in stbm_alloc");
        b->u.chunk = c;
    } else {
...
static void stbm__system_alloc_unlink(stbm_heap *heap, stbm__system_allocation *alloc, stbm__uint32 magic) {
    ...
    if (alloc->magic != magic) STBM_FAIL("Corrupt block in stbm_free");
}
...
static void stbm__large_free(stbm_heap *heap, void *ptr) {
    ...
    if (a->system_allocation->magic != STBM__SYSTEM_ALLOC_LARGE) STBM_FAIL("Corrupt block in stbm_free");
...
static void stbm__medium_unlink(stbm_heap *heap, struct stbm__medium_freeblock *block) {
    ...
    if (heap->medium.lists[index][slot] != block) {
        ...
        if (next)
            ...
            if (heap->medium.bitmap[index] == 0) heap->medium.toplevel_bitmap &= ~(1 << (31 - index));
        }
    }
}
...
static void *stbm__medium_alloc(stbm_heap *heap, size_t size) {
        ...
        if (index < 32) {
            ...
            if (heap->medium.cached_system_alloc_size >= STBM__MAX(freesize, size_with_overhead)) {
                ...
                if (size_with_overhead > freesize) {
                    ...
                    heap->cur_medium_chunk_size =
                        STBM__MIN(heap->cur_medium_chunk_size * 2, heap->max_medium_chunk_size);
                }
    ...
    alloc->prefix = STBM__SIZE_SETDATA(
        alloc->prefix, (is.index << 6) + (is.slot << 1) + (freesize == expected_size + STBM__EXTRA_SIZE ? 1 : 0));
...
static void stbm__medium_check_list(stbm_heap *heap, int index, int slot) {
    ...
    while (cur) {
        ...
        if (((stbm__uintptr *) cur)[-1] < STBM__MEDIUM_CHUNK_PRE_HEADER)
            if ((prefix & STBM__PREVIOUS_MASK) == STBM__PREVIOUS_free) STBM_FAIL(message);
        // the sizeclass in the block must match the passed-in sizeclass
        if (STBM__MED_FREE_SIZECLASS(prefix) != (unsigned) (index * STBM__NUM_SLOTS + slot)) STBM_FAIL(message);
        // the linked list must be linked properly
        if (cur->next_free && cur->next_free->prev_free != cur) STBM_FAIL(message);
        // it should have the right stored sizeclass
        if ((int) (STBM__MED_FREE_SIZECLASS(prefix) >> 5) != index ||
            (int) (STBM__MED_FREE_SIZECLASS(prefix) & 31) != slot)
            STBM_FAIL(message);
        ...
        if (((((stbm__medium_freeblock *) ((char *) cur + STBM__MED_FREE_BYTES(prefix)))->prefix) &
             STBM__PREVIOUS_MASK) != STBM__PREVIOUS_free)
            STBM_FAIL(message);
        cur = cur->next_free;
    }
}
...
static void stbm__tc_update_lowwater_mark(stbm__cache *sizecache) {
    if (sizecache->num < sizecache->lowwater) sizecache->lowwater = sizecache->num;
}
...
static void stbm__tc_alloc_cached_blocks(stbm_tc *tc, int tc_slot, int sizeclass) {
    ...
    if (!tc->cache[tc_slot].alloc)
        ...
        if (num_alloc >= 16 || size * num_alloc >= 65536) num_alloc = tc->cache[tc_slot].alloc;
    }
...
static void *stbm__tc_alloc(stbm__request *request) {
    ...
    if (tc && tc->heap == request->heap && request->size <= STBM__MAX_TC_ALLOC_SIZE) {
        ...
        if (tc_slot < 0) tc_slot = stbm__tc_allocate_slot(tc, tc_index, tc->heap);  // can't fail
...
static void stbm__tc_scavenge(stbm_tc *tc, stbm_heap *safe_heap) {
    ...
    for (i = 0; i < STBM__NUM_THREADCACHE; ++i) {
        if (tc->sizeclass[i] && tc->cache[i].num) {
            ...
            if (count) {
                if (count > tc->cache[i].num) count = tc->cache[i].num;
...
static void stbm__tc_free(stbm_tc *tc, stbm_heap *safe_heap, void *ptr) {
    ...
    if (tc && tc->heap == src_heap && size <= STBM__MAX_TC_ALLOC_SIZE) {
        ...
        ++sizecache->num;
        tc->cached_bytes += stbm__size_for_tc_index(tc_index);
        if (tc->cached_bytes >= tc->cached_bytes_scavenge_threshhold) stbm__tc_scavenge(tc, safe_heap);
    } else
...
static void *stbm__alloc_align(stbm__request *request) {
    ...
    void *user_ptr =
        stbm__alloc_aligned_internal(request, sizeof(stbm__aligned_allocated) + sizeof(stbm__aligned_base), 0, &base);
...
static void stbm__alloc_check_guard(stbm__debug_allocated *da, char *message) {
    ...
    if (num_bytes & 0xf00f) {
        ...
        for (i = 0; i < num_bytes; ++i)
            if (guard_area[i] != c) break;
        if (i < num_bytes) STBM_FAIL(message);
    }
}
...
STBM__API void *stbm_alloc(stbm_tc *tc, stbm_heap *heap, size_t size, unsigned short userdata16) {
    ...
    if (size <= STBM__SMALLEST_ALLOCATION)
        if (heap->align_all_blocks_to_minimum)
            if (heap->minimum_alignment > STBM__SMALL_ALIGNMENT) r.size = STBM__SMALLEST_ALLOCATION + 1;
...
STBM__API void *stbm_alloc_align(stbm_tc *tc, stbm_heap *heap, size_t size, unsigned short userdata16, size_t alignment,
    ...
    if (align_offset == 0 && alignment <= heap->minimum_alignment)
        if (size > STBM__SMALLEST_ALLOCATION || !heap->align_all_blocks_to_minimum) return stbm__alloc_normal(&r);
...
STBM__API size_t stbm_get_allocation_size(void *ptr) {
    ...
    if (STBM__TYPE(prefix) == STBM__TYPE_medium) {
        ...
        return stbm__size_for_index_slot(STBM__SIZEDATA_index(size), STBM__SIZEDATA_slot(size)) +
               extra[STBM__SIZEDATA_extra(size)] - sizeof(stbm__medium_allocated);
    }

    if (STBM__TYPE(prefix) == STBM__TYPE_small) return STBM__SMALLEST_BLOCKSIZE - sizeof(stbm__small_allocated);

    if (STBM__TYPE(prefix) == STBM__TYPE_large) return ((stbm__large_allocated *) ptr)[-1].user_size_in_bytes;

    if (STBM__TYPE(prefix) == STBM__TYPE_debug) return ((stbm__debug_allocated *) ptr)[-1].user_size_in_bytes;
...
stbm_heap *stbm_get_heap(void *ptr) {
    ...
    if (STBM__TYPE_med_large(prefix)) return ((stbm__medium_allocated *) ptr)[-1].heap;

    if (STBM__TYPE(prefix) == STBM__TYPE_small) return ((stbm__small_allocated *) ptr)[-1].u.chunk->heap;
...
STBM__API void stbm_heap_free(stbm_heap *heap) {
    ...
    if (heap->medium.cached_system_alloc) heap->system_free(heap->user_context, heap->medium.cached_system_alloc);
}

static void stbm__heap_check_locked(stbm_heap *heap) { stbm__medium_check_all(heap); }
...
STBM__API void stbm_heapconfig_gather_full_stats(stbm_heap *heap) { heap->full_stats = 1; }
...
STBM__API void stbm_heapconfig_set_small_chunk_size(stbm_heap *heap, size_t size) { heap->smallchunk_size = size; }
...
static void stbm__process_user_pointer(void *ptr, stbm_debug_iterate_func *callback, void *user_context,
                                       int maybe_wrapped) {
    stbm__uintptr prefix = ((stbm__uintptr *) ptr)[-1];
    ...
    if (callback) callback(user_context, &info);
}
...
STBM__API void stbm_debug_iterate(stbm_heap *heap, stbm_debug_iterate_func *callback, void *user_context) {
    ...
    while (sa != &heap->system_sentinel) {
        ...
        if (sa->magic == STBM__SYSTEM_ALLOC_LARGE) {
            ...
            if (user != sa->end_pointer) STBM_FAIL("Heap walk lengths didn't validate");
        }
        sa = sa->next;
    }
    STBM__RELEASE(heap->allocation_mutex);
...
static void stbm__test_heap(int alignment, int count) {
    ...
    for (i = 0; i < 5000; ++i) {
        ...
        if (allocations[i] == 0) i = i;
        STBM_ASSERT(stbm_get_heap(allocations[i]) == heap);
        r = stbm_get_allocation_size(allocations[i]);
        STBM_ASSERT(r >= size);
        sizes[i] = size;
        size = size + 1 + (size >> 4);
        if (size > (96 << 10)) size = 1;
        for (k = 0; k <= i; ++k) STBM_ASSERT(stbm_get_allocation_size(allocations[k]) >= sizes[k]);
    }
    stbm_debug_iterate(heap, 0, 0);
    for (i = 0; i < 5000; ++i) stbm_free(0, heap, allocations[i]);

    printf("Made %d system allocations total; %d outstanding\n", stbm__num_sys_alloc,
           stbm__num_sys_alloc - stbm__num_sys_free);
    ...
    for (i = 0; i < 5000; ++i) allocations[i] = stbm_alloc(0, heap, 1, 0);
    stbm_debug_iterate(heap, 0, 0);
    for (i = 0; i < 5000; i += 2) stbm_free(0, heap, allocations[i]);
    for (i = 0; i < 5000; i += 2) stbm_free(0, heap, allocations[i + 1]);
    ...
    // test large blocks
    for (i = 0; i < 500; ++i) allocations[i] = stbm_alloc(0, heap, (i % 2 + 1) << 20, (unsigned short) i);
    ...
    for (i = 0; i < 500; ++i) stbm_free(0, heap, allocations[i]);
    ...
    for (i = 0; i < count; ++i) {
        ...
        if (allocations[j]) {
            for (k = 0; k < (int) sizes[j]; ++k) STBM_ASSERT(((char *) allocations[j])[k] == (char) j);
        ...
        if (i % 3333 == 0) stbm_debug_iterate(heap, 0, 0);
    }
    ...
    if (stbm__num_sys_alloc == stbm__num_sys_free) STBM_FAIL("weird");
    stbm_heap_free(heap);
    if (stbm__num_sys_alloc != stbm__num_sys_free) STBM_FAIL("heap free failed to free everything");
}
...
static void stbm__test_heap_sorted(void) {
    ...
    for (j = 0; j < 50; ++j) {
        for (i = 0; i < 3000; ++i) allocations[i] = stbm_alloc(0, heap, size + (i % 128), (unsigned short) i);
    ...
    for (j = 0; j < 10; ++j) {
        ...
        for (i = 0; i < 5000; ++i) allocations[i] = stbm_alloc(0, heap, size + (i % 128), (unsigned short) i);
        for (i = 0; i < 5000; i += 2) stbm_free(0, heap, allocations[i]);
        for (i = 1; i < 5000; i += 2) stbm_free(0, heap, allocations[i]);
    ...
    for (j = 0; j < 10; ++j) {
        for (i = 0; i < 5000; ++i) allocations[i] = stbm_alloc(0, heap, size + (i % 128), (unsigned short) i);
        qsort(allocations, sizeof(allocations[0]), 5000, stbm__ptr_compare);
        for (i = 0; i < 5000; i += 3) stbm_free(0, heap, allocations[i]);
        for (i = 1; i < 5000; i += 3) stbm_free(0, heap, allocations[i]);
        for (i = 2; i < 5000; i += 3) stbm_free(0, heap, allocations[i]);
        printf("Made %d system allocations total; %d outstanding\n", stbm__num_sys_alloc,
               stbm__num_sys_alloc - stbm__num_sys_free);
    }
