...
extern void win32_register_window_class();
...
extern wchar_t *g_Win32WindowClassName;
...
void win32_monitor_init() {
    ...
    if (shcore.hInstance) {
        shcore.SetProcessDpiAwareness_ =
            (PFN_SetProcessDpiAwareness) GetProcAddress(shcore.hInstance, "SetProcessDpiAwareness");
    ...
    if (ntdll.hInstance) {
        ntdll.RtlVerifyVersionInfo_ =
            (PFN_RtlVerifyVersionInfo) GetProcAddress(ntdll.hInstance, "RtlVerifyVersionInfo");
    }
    ...
    // Create helper window
    {
        ...
        HelperWindowHandle =
            CreateWindowExW(WS_EX_OVERLAPPEDWINDOW, g_Win32WindowClassName, L"GLFW message window",
...
static win32_monitor_state STATE;
