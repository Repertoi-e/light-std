...
void print_error_description(HRESULT hr) {
    ...
    switch (hr) {
        ...
        CHK_ERR(DDERR_OVERLAPPINGRECTS,
                "Operation could not be carried out because the source and destination rectangles are on the same "
        ...
        CHK_ERR(DDERR_CANTLOCKSURFACE,
                ...
                "surface. This is most likely to happen when attempting to lock the primary surface when no DCI "
        ...
        CHK_ERR(DDERR_NOOVERLAYDEST,
                "returned when GetOverlayPosition is called on a overlay that UpdateOverlay has never been called on "
        ...
        CHK_ERR(DDERR_CANTCREATEDC,
                "Windows can not create any more DCs, or a DC was requested for a paltte-indexed surface when the "
                "surface had no palette AND the display mode was not palette-indexed (in this case DirectDraw cannot "
                "select a proper palette into the DC)")
        ...
        CHK_ERR(DXGI_ERROR_INVALID_CALL,
                ...
                "intended to denote that the application should be altered to avoid the error. Use of the debug "
                "version of the DXGI.DLL will provide run-time debug output with further information.")
        CHK_ERR(DXGI_ERROR_NOT_FOUND,
                "The item requested was not found. For GetPrivateData calls, this means that the specified GUID had "
        ...
        CHK_ERR(DXGI_ERROR_NONEXCLUSIVE,
                "The application attempted to perform an operation on an DXGI output that is only legal after the "
                "output has been claimed for exclusive owenership.")
        ...
        CHK_ERR(D2DERR_RECREATE_TARGET,
                "here has been a presentation error that may be recoverable. The caller needs to recreate, rerender "
                "the entire frame, and reattempt present.")
        ...
        CHK_ERR(DWRITE_E_UNEXPECTED,
                "Indicates an error originating in DirectWrite code, which is not expected to occur but is safe to "
                "recover from.")
        CHK_ERR(DWRITE_E_NOFONT, "Indicates the specified font does not exist.")
        CHK_ERR(DWRITE_E_FILENOTFOUND,
                "A font file could not be opened because the file, directory, network location, drive, or other "
                "storage location does not exist or is unavailable.")
...
void windows_report_hresult_error(long hresult, string call, string file, s32 line) {
    fmt::print("\n{!}(windows_error.cpp): An error occured when calling a function returning an HRESULT.\n");
    fmt::print("    {!GRAY}{}{!}\n", call);
