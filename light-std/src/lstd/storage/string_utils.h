...
// Retrieve the length of a standard cstyle string (just like strlen)
...
constexpr void to_utf16(const byte *str, size_t length, wchar_t *out) {
    auto *p = out;
    For(range(length)) {
...
// Assumes _out_ has enough space to hold the converted bytes
constexpr void to_utf32(const byte *str, size_t length, char32_t *out) {
    auto *p = out;
    For(range(length)) {
        char32_t it = decode_cp(str);
        *p++ = it;
