...
// (for example if you want to use it as a key in a Table), implement it like this:
//
// template <>
// struct hash<my_type> {
//     static uptr_t get(my_type value) { return ...; }
// }
//
...
template <typename T>
struct hash {
    static constexpr uptr_t get(const T &value);
};
...
struct hash<f32> {
    static constexpr uptr_t get(f32 value) {
        // Instead of a normal reinterpret cast we use an union, because the former doesn't compile on clang
        union f32_u32 {
            f32 f;
            u32 u;
