...
//  in imgui.cpp. [X] Renderer: Multi-viewport support. Enable with 'io.ConfigFlags |=
//  ImGuiConfigFlags_ViewportsEnable'. [X] Renderer: Support for large meshes (64k+ vertices) with 16-bits indices.
...
//  2019-07-21: DirectX11: Backup, clear and restore Geometry Shader is any is bound when calling
//  ImGui_ImplDX10_RenderDrawData. Clearing Hull/Domain/Compute shaders without backup/restore. 2019-05-29: DirectX11:
//  Added support for large mesh (64K+ vertices), enable ImGuiBackendFlags_RendererHasVtxOffset flag. 2019-04-30:
//  DirectX11: Added support for special ImDrawCallback_ResetRenderState callback to reset render state. 2018-12-03:
//  Misc: Added #pragma comment statement to automatically link with d3dcompiler.lib when using D3DCompile().
...
void ImGui_ImplDX11_RenderDrawData(ImDrawData *draw_data) {
    ...
    if (ctx->Map(g_pVB, 0, D3D11_MAP_WRITE_DISCARD, 0, &vtx_resource) != S_OK) return;
    if (ctx->Map(g_pIB, 0, D3D11_MAP_WRITE_DISCARD, 0, &idx_resource) != S_OK) return;
    ...
    // draw_data->DisplayPos+data_data->DisplaySize (bottom right). DisplayPos is (0,0) for single viewport apps.
    {
        ...
        if (ctx->Map(g_pVertexConstantBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &mapped_resource) != S_OK) return;
...
bool ImGui_ImplDX11_Init(ID3D11Device *device, ID3D11DeviceContext *device_context) {
    ...
    if (io.ConfigFlags & ImGuiConfigFlags_ViewportsEnable) ImGui_ImplDX11_InitPlatformInterface();
...
static void ImGui_ImplDX11_RenderWindow(ImGuiViewport *viewport, void *) {
    ...
    g_pd3dDeviceContext->OMGetRenderTargets(1, &pRTV, null);
    defer({
        if (pRTV) pRTV->Release();
    });
    if (!(viewport->Flags & ImGuiViewportFlags_NoRendererClear)) {
        g_pd3dDeviceContext->ClearRenderTargetView(pRTV, (float *) &clear_color);
    }
