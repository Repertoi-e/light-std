...
inline void fmt_context::write(const char *data, s64 count) { write_helper(this, data, count); }
...
void write_u64(fmt_context *f, u64 value, bool negative, fmt_specs specs) {
    ...
    if (type == 'n') { formattedSize += ((numDigits - 1) / 3); }
    ...
    write_padded_helper(
        ...
        [&]() {
            ...
            if (type == 'd') {
                ...
                p = format_uint_base<4>(U64_FORMAT_BUFFER, value, numDigits, ascii_is_upper(specs.Type));
            } else if (type == 'n') {
...
void write_float_fixed(fmt_context *f, string significand, s32 exp, code_point sign, const fmt_specs &specs, const fmt_float_specs &floatSpecs, bool percentage) {
    s64 outputSize = (sign ? 1 : 0) + (percentage ? 1 : 0) + significand.Count;  // Further down we add the number of extra
    ...
    if (exp >= 0) {
        ...
        write_padded_helper(
            ...
            [&]() {
                ...
                write_significand(f, significand,
        ...
        if (absExp < significand.Count) {
            ...
            if (!significand.Count && specs.Precision >= 0 && specs.Precision < numZeros) { numZeros = specs.Precision; }
...
fmt_float_specs fmt_parse_float_specs(fmt_parse_context *p, fmt_specs no_copy specs) {
    ...
    switch (specs.Type) {
        case 0:
            ...
            break;
        case 'G': result.Upper = true; [[fallthrough]];
        case 'g': result.Format = fmt_float_specs::GENERAL; break;
        case 'E': result.Upper = true; [[fallthrough]];
        case 'e':
            result.Format = fmt_float_specs::EXP;
            result.ShowPoint |= specs.Precision != 0;
            break;
        case 'F': result.Upper = true; [[fallthrough]];
        ...
        case 'f':
            ...
            result.ShowPoint |= specs.Precision != 0;
            break;
        case 'A': result.Upper = true; [[fallthrough]];
        case 'a': result.Format = fmt_float_specs::HEX; break;
        default:  p->on_error("Invalid type specifier for a f32", p->It.Data - p->FormatString.Data - 1); break;
    }
    return result;
}
