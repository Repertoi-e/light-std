...
array<os_function_call> os_get_call_stack(s32 skipFrames, s32 maxDepth,
  ...
  auto push_frame = [&](DWORD64 address) {
    ...
    if (symbolsReady) {
      ...
      if (SymGetLineFromAddrW64(process, address, &lineDisplacement,
                                &lineInfo)) {
  ...
  if (platformContext) {
    ...
    while ((s32)callStack.Count < maxDepth &&
           StackWalk64(IMAGE_FILE_MACHINE_AMD64, process, GetCurrentThread(),
    ...
    ULONG captured = RtlCaptureStackBackTrace((ULONG)skipFrames + 1,
                                             (ULONG)maxDepth, frames, null);
    for (ULONG i = 0; i < captured && (s32)callStack.Count < maxDepth; ++i) {
      push_frame((DWORD64)frames[i]);
    }
  }
