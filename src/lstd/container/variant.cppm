...
// optional<> is a type that either holds T or doesn't (implementation-wise it's just a variant between T and a null type)
//
...
//     [](auto) { print("No number\n"); }               // If the default statement is not handled in  match, there will be a compiler error
// });
// 
// 
...
//
template <class U, class T> struct const_if_impl { using type = T; };
template <class U, class T> struct const_if_impl<U const*, T> { using type = const T; };
...
template <class U, class ...TS>
constexpr auto index_of = checked<
	decay_t<U>,
	decay_t<TS>...
>;
...
export {
	...
	struct aligned_union
	{
		...
		aligned_union(T&& x) { construct<T>(FORWARD(x)); }
		aligned_union() : Data{} { }
	...
	struct variant
	{
		...
		template <class T>
		auto is() const { return (get_index_of_t<T> == ti); }
		...
		{
			if (is<T>()) as<T>() = FORWARD(x);
			else emplace<T>(FORWARD(x));
			return *this;
		}
	...
	private:
		...
		decltype(auto) visit(F no_copy f, u64) { return f(as<T>()); }

		template <class F, class T>
		decltype(auto) visit(F no_copy f, u64) const { return f(as<T>()); }
		...
		{
			template <class T>
			void operator () (T ref x) const { x.~T(); }
		};
		...
		struct move_constructor
		{
			...
			template <class T>
			void operator () (T& x) const { self.construct<T>(MOVE(x)); }
		};
		...
		struct copy_constructor
		{
			...
			template <class T>
			void operator () (T no_copy x) const { self.construct<T>(x); }
		};
	};
