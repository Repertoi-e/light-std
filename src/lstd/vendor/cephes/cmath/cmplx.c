...
void cadd(a, b, c) register cmplx *a, *b;
cmplx *c;
{
  c->r = b->r + a->r;
  c->i = b->i + a->i;
...
void csub(a, b, c) register cmplx *a, *b;
cmplx *c;
{
  c->r = b->r - a->r;
  c->i = b->i - a->i;
...
void cmul(a, b, c) register cmplx *a, *b;
cmplx *c;
{
  double y;
...
void cdiv(a, b, c) register cmplx *a, *b;
cmplx *c;
{
  double y, p, q, w;
...
double cabs(z) register cmplx *z;
{
  double x, y, b, re, im;
  int ex, ey, e;
  ...
  if (z->r == INFINITY || z->i == INFINITY || z->r == -INFINITY ||
      z->i == -INFINITY)
    return INFINITY;
#endif

#ifdef NANS
  if (isnan(z->r)) return z->r;
  if (isnan(z->i)) return z->i;
#endif
