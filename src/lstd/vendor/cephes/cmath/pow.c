...
double pow(x, y) double x, y;
{
...
#else
    if (x > 0.0 && x < 1.0) return (MAXNUM);
#endif
    if (x < -1.0) return 0.0;
#ifdef INFINITIES
    if (x > -1.0 && x < 0.0) return INFINITY;
#else
    if (x > -1.0 && x < 0.0) return (MAXNUM);
#endif
  }
  if (x >= MAXNUM) {
#if INFINITIES
  ...
  if (x <= -MAXNUM) {
    if (y > 0.0) {
      ...
      return INFINITY;
#else
      if (yoddint) return (-MAXNUM);
      return (MAXNUM);
#endif
    }
    if (y < 0.0) {
#ifdef MINUSZERO
      if (yoddint) return NEGZERO;
#endif
      return 0.0;
    }
  }
  ...
  if (x <= 0.0) {
    if (x == 0.0) {
      if (y < 0.0) {
#ifdef MINUSZERO
        if (signbit(x) && yoddint) return -INFINITY;
#endif
#ifdef INFINITIES
        return INFINITY;
#else
      ...
      if (y > 0.0) {
#ifdef MINUSZERO
        if (signbit(x) && yoddint) return NEGZERO;
#endif
        return 0.0;
      }
      return 1.0;
...
#endif
#ifdef INFINITIES
    if (nflg && yoddint) return -INFINITY;
    return INFINITY;
#else
    if (nflg && yoddint) return (-MAXNUM);
    return (MAXNUM);
#endif
  }
...
#endif
#ifdef MINUSZERO
    if (nflg && yoddint) return NEGZERO;
#endif
    return 0.0;
  }
