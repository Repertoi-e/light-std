...
#include "arche/vendor/tlsf/tlsf.h"

#include "arche/fmt.h"

// :WEMODIFIED: We replaced printf with our print.
using ARCHE_NAMESPACE::print;
...
extern "C" unsigned char __cdecl _BitScanForward(unsigned long *_Index, unsigned long _Mask);
extern "C" unsigned char __cdecl _BitScanReverse(unsigned long *_Index, unsigned long _Mask);
...
static const u64 block_start_offset = offset_of(block_header_t, size) + sizeof(u64);
...
** bits for FL_INDEX.
*/
static const u64 block_size_min = sizeof(block_header_t) - sizeof(block_header_t *);
...
static u64 block_size(const block_header_t *block) { return block->size & ~(block_header_free_bit | block_header_prev_free_bit); }
...
static block_header_t *offset_to_block(const void *ptr, u64 size) { return tlsf_cast(block_header_t *, tlsf_cast(tlsfptr_t, ptr) + size); }
...
static int block_can_split(block_header_t *block, u64 size) { return block_size(block) >= sizeof(block_header_t) + size; }
...
*/
u64 tlsf_size(void) { return sizeof(control_t); }

u64 tlsf_align_size(void) { return ALIGN_SIZE; }

u64 tlsf_block_size_min(void) { return block_size_min; }

u64 tlsf_block_size_max(void) { return block_size_max; }
...
** sentinel block.
*/
u64 tlsf_pool_overhead(void) { return 2 * block_header_overhead; }

u64 tlsf_alloc_overhead(void) { return block_header_overhead; }
...
int test_ffs_fls() {
    ...
    if (rv) { print("test_ffs_fls: {:x} ffs/fls tests failed.\n", rv); }
    return rv;
}
#endif
...
void *tlsf_memalign(tlsf_t tlsf, u64 align, u64 size) {
  ...
  */
    ...
    if (block) {
        ...
        u64   gap     = tlsf_cast(u64, tlsf_cast(tlsfptr_t, aligned) - tlsf_cast(tlsfptr_t, ptr));
        ...
        if (gap && gap < gap_minimum) {
            ...
            gap     = tlsf_cast(u64, tlsf_cast(tlsfptr_t, aligned) - tlsf_cast(tlsfptr_t, ptr));
        }
...
** We also changed the name of this routine to be more clear with what we are
*doing.
** This may fail and returns a null pointer. Reallocation is handled up in the
*call
...
void *tlsf_resize(tlsf_t tlsf, void *ptr, u64 size) {
    ...
    ** resize function which fails if there is not enough space and returns
    *null.
    ** The reallocate_array function handles allocating a new block and all that
    *jazz.
    */
