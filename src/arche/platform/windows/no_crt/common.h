...
// These tables are filled by the linker and are usually called before the main
// function by the CRT. Since we don't have the CRT we have to call them
// manually.

//
// Section attributes
// Taken from: "vcruntime/internal_shared.h", commented out stuff we don't need.
// I still don't know what pogo is...
//
...
// #pragma section(".CRT$XIYA", long, read)  // VCCorLib Threading Model
// Initializer #pragma section(".CRT$XIYAA", long, read)  // XAML Designer
// Threading Model Override Initializer #pragma section(".CRT$XIYB", long, read)
// // VCCorLib Main Initializer
...
// #pragma section(".CRTVT$XCA", long, read)  // First Managed VTable
// Initializer #pragma section(".CRTVT$XCZ", long, read)  // Last Managed VTable
// Initializer
...
// We prefix these with "arche_" because Windows.h includes corecrt_startup.h
// with declares _initterm_ and _initterm_e_ as dllimport. Sigh.
extern "C" void __cdecl arche_initterm(_PVFV *first, _PVFV *last);
extern "C" int __cdecl  arche_initterm_e(_PIFV *first, _PIFV *last);
