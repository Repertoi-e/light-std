...
// * Tests whether a PE image is located at the given image base.  Returns true
// if
...
// * image located at __ImageBase), that the target address is located in a
// proper
...
/***
 *tlsdyn.cpp - Thread Local Storage dynamic initialization run-time support
 *module
 *
...
#pragma warning(pop)
    ...
    //
    // If you use arche's API for creating a thread then we CAN know,
    ...
    if ((void *)&ARCHE_NAMESPACE::Context != MainContext) { memset((byte *)&ARCHE_NAMESPACE::Context, ARCHE_NAMESPACE::DEAD_LAND_FILL, sizeof(ARCHE_NAMESPACE::Context)); }
}
