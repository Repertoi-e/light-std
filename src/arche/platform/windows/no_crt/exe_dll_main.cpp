...
#include "arche/os.h"
#include "arche/os/windows/api.h"  // For definitions

#if OS != WINDOWS
#error ARCHE_NO_CRT is Windows-only
#endif
...
extern "C" void main_no_crt() {
    ...
    if (arche_initterm_e(__xi_a, __xi_z) != 0) {
        debug_break();
        return;
    }
    arche_initterm(__xc_a, __xc_z);

    platform_state_init();

    // We do this to avoid reinitializing in __dyn_tls_init (in tlsdyn.cpp)
    MainContext = (void *)&ARCHE_NAMESPACE::Context;
    ...
    ARCHE_NAMESPACE::exit(mainResult);
}
...
static BOOL __cdecl dllmain_crt_process_detach(bool const is_terminating) {
    ...
    ARCHE_NAMESPACE::exit_call_scheduled_functions();
    return 1;
}
