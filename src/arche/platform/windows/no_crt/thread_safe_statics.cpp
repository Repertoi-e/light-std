...
// On Vista or newer, the native CONDITION_VARIABLE type is used.  On XP, we use
// a simple Windows event.  This is not safe to use as a complete condition
// variable, but for the purposes of this feature the event is sufficient but
// not optimal.  See the code in _Tss_wait below.
//
...
static void __cdecl __scrt_initialize_thread_safe_statics_platform_specific() noexcept {
    ...
    if (kernel_dll == null) { kernel_dll = GetModuleHandleW(L"kernel32.dll"); }

    if (kernel_dll == null) {
        // __scrt_fastfail(FAST_FAIL_FATAL_APP_EXIT);
        ARCHE_NAMESPACE::exit(-1);
    }
...
static void __cdecl __scrt_uninitialize_thread_safe_statics() noexcept {
    ...
    if (g_tss_event != null) { CloseHandle(g_tss_event); }
}
...
static int __cdecl __scrt_initialize_thread_safe_statics() noexcept {
    ...
    // If CRT initialization was skipped then we should initialize the atexit
    // tables. This will only be needed when using a managed DLL with /NOENTRY
    // specified. if (!__scrt_initialize_onexit_tables(__scrt_module_type::dll)) {
    //     __scrt_fastfail(FAST_FAIL_FATAL_APP_EXIT);
    // }
    ARCHE_NAMESPACE::exit_schedule(__scrt_uninitialize_thread_safe_statics);
    return 0;
}
...
// ones that use the most efficient synchronization primitives available on the
// platform. This is not intended to be a recursive lock.
...
// Windows event we can't guarantee that we'll ever actually receive the
// notification signal, so we must use a non-infinite timeout.  This is not
// optimal: we may wake up early if the initializer is long-running, or we may
// miss the signal and not wake up until the timeout expires.  The signal may be
// missed because the sleeping threads may be stolen by the kernel to service an
// APC, or due to the race condition between the unlock call and the
