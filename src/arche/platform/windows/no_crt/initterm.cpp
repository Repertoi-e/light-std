...
//
// These are prefixed with arche, see note in common.h in this directory.
//

// * Calls each function in [first, last).  [first, last) must be a valid range
// of
// * function pointers.  Each function is called, in order.
extern "C" void __cdecl arche_initterm(_PVFV *const first, _PVFV *const last) {
...
// * is returned.
// *
// * If a nonzero value is returned, it is expected to be one of the runtime
// error
// * values (_RT_{NAME}, defined in the internal header files).
extern "C" int __cdecl arche_initterm_e(_PIFV *const first, _PIFV *const last) {
