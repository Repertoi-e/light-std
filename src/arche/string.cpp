...
static inline u32 clamp_cp(code_point cp) {
    if (cp < 0) return 0;
    if ((u32)cp >= UNICODE_TABLE_SIZE) return (u32)cp;  // out of compiled range; identity
    return (u32)cp;
}
...
code_point unicode_to_upper(code_point cp, text_locale loc) {
    if (loc == text_locale::Unspecified) loc = Context.Locale;
    u32 c = clamp_cp(cp);
    if (c >= UNICODE_TABLE_SIZE) return cp;  // identity beyond table
    // Turkic: i (0069) -> İ (0130)
    if (loc == text_locale::Turkic && c == 0x0069) return 0x0130;
...
u8 unicode_combining_class(code_point cp) {
    u32 c = clamp_cp(cp);
    if (c >= UNICODE_TABLE_SIZE) return 0;
...
s32 unicode_canonical_decompose(code_point cp, code_point *out, s32 cap) {
    ...
    s32 n   = (s32)len;
    for (s32 i = 0; i < n && i < cap; ++i) { out[i] = (code_point)g_unicode_decomp_array[idx + 1 + (u32)i]; }
    return n;
}
...
code_point unicode_compose_pair(code_point a, code_point b) {
    ...
    while (lo <= hi) {
        ...
        else return (code_point)g_unicode_comp_values[mid];
    }
    return 0;
}
