...
#include "arche/parse.h"

ARCHE_BEGIN_NAMESPACE

enum emphasis : u8 { BOLD = BIT(0), ITALIC = BIT(1), UNDERLINE = BIT(2), STRIKETHROUGH = BIT(3) };
...
inline string terminal_color_to_string(terminal_color c) {
    switch (c) {
#define COLOR_DEF(x, y) \
    case terminal_color::x: return string(#x);
#include "terminal_colors.inl"
#undef COLOR_DEF
        default: return "NONE";
    }
...
bool handle_emphasis(fmt_parse_context *p, fmt_text_style *textStyle) {
    while (p->It.Count && ascii_is_alpha(p->It[0])) {
        switch (p->It[0]) {
            case 'B': textStyle->Emphasis |= BOLD; break;
            case 'I': textStyle->Emphasis |= ITALIC; break;
            case 'U': textStyle->Emphasis |= UNDERLINE; break;
            case 'S': textStyle->Emphasis |= STRIKETHROUGH; break;
            default:
                p->on_error(
                    "Invalid emphasis character - valid ones are: B (bold), I (italic), U (underline) and S (strikethrough). If you meant to use a terminal color, prefix it with "
                    "'t', because it's different than a color name, which uses RGB escape codes.");
                return false;
        }
...
fmt_parse_text_style_result fmt_parse_text_style(fmt_parse_context *p) {
    ...
    if (ascii_is_alpha(p->It[0])) {
        ...
        s64         n  = p->It.Count;
        do { ++it, --n; } while (n && ascii_is_identifier_start(*it));
        if (!n) return {true, textStyle};
...
inline bool type_selector_compatible(char typeChar, fmt_type k) {
    if (typeChar == 0) return true;
    switch (k) {
        case fmt_type::F32:
        case fmt_type::F64: return typeChar == 'f' || typeChar == 'F' || typeChar == 'g' || typeChar == 'G' || typeChar == 'e' || typeChar == 'E' || typeChar == '%';
        case fmt_type::S64:
        case fmt_type::U64:
        case fmt_type::BOOL:
            return typeChar == 'd' || typeChar == 'x' || typeChar == 'X' || typeChar == 'o' || typeChar == 'b' || typeChar == 'B' || typeChar == 'c' || typeChar == 'n';
        case fmt_type::STRING:  return typeChar == 's' || typeChar == 'q' || typeChar == 'p';
        case fmt_type::POINTER: return typeChar == 'p';
        case fmt_type::CUSTOM:  return true;
        default:                return false;
    }
}

fmt_dynamic_specs fmt_forwarded_specs_for_arg(fmt_dynamic_specs original, fmt_arg ar) {
    if (ar.Type == fmt_type::CUSTOM) return create_forwarded_specs(original);
    if (type_selector_compatible(original.Type, ar.Type)) return create_forwarded_specs(original);
...
void write_arg_with_forwarding_pretty(fmt_context *F, fmt_arg ar, bool noSpecs, s32 indentSize, s32 nextLevel) {
    ...
    if (original) {
        auto sp = [&]() -> fmt_dynamic_specs {
            auto t     = (ar.Type == fmt_type::CUSTOM || type_selector_compatible(original->Type, ar.Type)) ? create_forwarded_specs(*original) : create_safe_specs(*original);
...
void fmt_parse_context::require_arithmetic_arg(fmt_type argType, s64 errorPosition) {
    ...
    if (!fmt_type_is_arithmetic(argType)) on_error("Format specifier requires an arithmetic argument", errorPosition);
}
...
void fmt_parse_context::require_signed_arithmetic_arg(fmt_type argType, s64 errorPosition) {
    ...
    if (fmt_type_is_integral(argType) && argType != fmt_type::S64) { on_error("Format specifier requires a signed integer argument (got unsigned)", errorPosition); }
}
...
void fmt_parse_context::check_precision_for_arg(fmt_type argType, s64 errorPosition) {
    assert(argType != fmt_type::NONE);
    if (argType == fmt_type::CUSTOM) return;
    if (fmt_type_is_integral(argType)) { on_error("Precision is not allowed for integer types", errorPosition); }
    if (argType == fmt_type::POINTER) { on_error("Precision is not allowed for pointer type", errorPosition); }
}
...
bool parse_width(fmt_parse_context *p, fmt_dynamic_specs *specs) {
    if (ascii_is_digit(p->It[0])) {
        ...
        if (p->It.Count) {
            ...
            if (specs->WidthIndex == -1) return false;
        }
        if (!p->It.Count || p->It[0] != '}') {
...
bool parse_precision(fmt_parse_context *p, fmt_type argType, fmt_dynamic_specs *specs) {
    ...
    if (ascii_is_digit(p->It[0])) {
        ...
        if (p->It.Count) {
            ...
            if (specs->PrecisionIndex == -1) return false;
        }
        if (!p->It.Count || p->It[0] != '}') {
