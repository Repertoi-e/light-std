...
{
    g_active_tokenizer = tz;
    g_diag_get_line = lf;
    g_diag_get_filename = ff;
}
...
inline void diag_error(const char *title)
{
    ...
    else
    {
        ...
        if (g_active_tokenizer && g_diag_get_filename)
        {
            const char *fn = g_diag_get_filename(g_active_tokenizer);
            if (fn)
                snippet_set_path(sn, fn);
        }
...
{
    if (!g_active_tokenizer || !g_diag_get_line)
        return 1;
    s64 tokenizer_line = g_diag_get_line(g_active_tokenizer);
    s64 lines_before = 0;
    for (const char *p = g_source; p < pos && *p; ++p)
        if (*p == '\n')
            ++lines_before;
    s64 baseline = tokenizer_line - lines_before;
    if (baseline < 1)
        baseline = 1;
    return (int)baseline;
}
...
inline void ERR_ANNOTATED_CONTEXT(const char *title, const char *estart, const char *eend, const char *emsg, const char *cstart, const char *cend, const char *cmsg)
{
    diag_error_annotated_context(title, estart, eend, emsg, cstart, cend, cmsg);
}
inline void WARN_ANNOTATED_CONTEXT(const char *title, const char *estart, const char *eend, const char *emsg, const char *cstart, const char *cend, const char *cmsg)
{
    diag_warning_annotated_context(title, estart, eend, emsg, cstart, cend, cmsg);
}

inline void ERR_ANNOTATED(const char *title, s64 start, s64 end, const char *msg)
{
    if (!g_source)
        g_source = "";
    diag_error_annotated(title, g_source + start, g_source + end, msg);
}
inline void WARN_ANNOTATED(const char *title, s64 start, s64 end, const char *msg)
{
    if (!g_source)
        g_source = "";
    diag_warning_annotated(title, g_source + start, g_source + end, msg);
}
inline void ERR_ANNOTATED_CONTEXT(const char *title,
                                      s64 estart, s64 eend, const char *emsg,
                                      s64 cstart, s64 cend, const char *cmsg)
{
    if (!g_source)
        g_source = "";
    diag_error_annotated_context(title, g_source + estart, g_source + eend, emsg, g_source + cstart, g_source + cend, cmsg);
}
                                       ...
                                       s64 cstart, s64 cend, const char *cmsg)
{
    if (!g_source)
        g_source = "";
    diag_warning_annotated_context(title, g_source + estart, g_source + eend, emsg, g_source + cstart, g_source + cend, cmsg);
}
