...
You need a compiler which fully supports C++20 modules and concepts, e.g. Visual Studio 2019 16.11 and above, Clang 14 and above.

### Windows
Run `Scripts/GenerateProjects.bat vs2019` (or vs2022). The solution file is at placed at `Build/vs2019/light-std.sln` (or `Build/vs2022/...`) and is generated by our bundled version of premake5 (`ThirdParty/bin/premake/`).

### Linux

Run `bash Scripts/GenerateProjects.sh` which uses our bundled version of premake5 (`ThirdParty/bin/premake/`) and `cmake` generator to output CMake files. CMake is needed to generate a `.modulemap` file, containing the module dependencies, so the compiler knows in which order to build the files.

Requirements are `sudo apt install clang llvm cmake libncurses5`.
