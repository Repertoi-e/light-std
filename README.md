...
You can also call with `--lstd-windows-link-runtime-library`. For example `.\scripts\GenerateProjects.bat vs2019 --lstd-windows-link-runtime-library` (anything after the target (vs2019) gets passed to premake as an option). This will force linking the runtime library on Windows. By default, we don't and we try our hardest to provide common functionality to external libraries that may require it (for example functions like `strncpy`, defined in `lstd/platform/windows/no_crt/common_functions.h/.cpp`. If a library requires a function you can add it there.
...
> Currently untested/unsupported.
