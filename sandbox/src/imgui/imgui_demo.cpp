...
static void ShowHelpMarker(char const *desc) {
    ImGui::TextDisabled("(?)");
...
void ImGui::ShowUserGuide() {
    ...
    ImGui::BulletText("CTRL+A or f64-click to select all.");
...
void ImGui::ShowDemoWindow(bool* p_open) {
    ...
    if (ImGui::CollapsingHeader("Widgets")) {
        if (ImGui::TreeNode("Basic")) {
            ...
            if (ImGui::IsItemHovered()) {
                ...
                static f32 arr[] = { 0.6f, 0.1f, 1.0f, 0.5f, 0.92f, 0.1f, 0.2f };
...

            {
                ...
                char const *items[] = { "AAAA", "BBBB", "CCCC", "DDDD", "EEEE", "FFFF", "GGGG", "HHHH", "IIII", "JJJJ", "KKKK", "LLLLLLL", "MMMM", "OOOOOOO" };
...

            {
                ...
                ImGui::SameLine(); ShowHelpMarker("Hold SHIFT or use mouse to select text.\n" "CTRL+Left/Right to word jump.\n" "CTRL+A or f64-click to select all.\n" "CTRL+X,CTRL+C,CTRL+V clipboard.\n" "CTRL+Z,CTRL+Y undo/redo.\n" "ESCAPE to revert.\n");

                ImGui::InputInt("input int", &i0);
                ImGui::SameLine(); ShowHelpMarker("You can apply arithmetic operators +,*,/ on numerical values.\n  e.g. [ 100 ], input \'*2\', result becomes [ 200 ]\nUse +- to subtract.\n");

                static f32 f0 = 0.001f;
                ImGui::InputFloat("input f32", &f0, 0.01f, 1.0f);

                static f64 d0 = 999999.00000001;
                ImGui::InputDouble("input f64", &d0, 0.01f, 1.0f, "%.8f");

                static f32 f1 = 1.e10f;
                ImGui::InputFloat("input scientific", &f1, 0.0f, 0.0f, "%e");
                ImGui::SameLine(); ShowHelpMarker("You can input value using the scientific notation,\n  e.g. \"1e+8\" becomes \"100000000\".\n");

                static f32 vec4a[4] = { 0.10f, 0.20f, 0.30f, 0.44f };
                ImGui::InputFloat3("input f323", vec4a);
            }

            {
                ...
                static f32 f1=1.00f, f2=0.0067f;
                ImGui::DragFloat("drag f32", &f1, 0.005f);
                ImGui::DragFloat("drag small f32", &f2, 0.0001f, 0.0f, 0.0f, "%.06f ns");
            }

            {
                ...
                static f32 f1=0.123f, f2=0.0f;
                ImGui::SliderFloat("slider f32", &f1, 0.0f, 1.0f, "ratio = %.3f");
                ImGui::SliderFloat("slider f32 (curve)", &f2, -10.0f, 10.0f, "%.4f", 2.0f);
                static f32 angle = 0.0f;
                ImGui::SliderAngle("slider angle", &angle);
            }

            {
                static f32 col1[3] = { 1.0f,0.0f,0.2f };
                static f32 col2[4] = { 0.4f,0.7f,0.0f,0.5f };
            ...
            {
                // List box
                char const *listbox_items[] = { "Apple", "Banana", "Cherry", "Kiwi", "Mango", "Orange", "Pineapple", "Strawberry", "Watermelon" };
        ...
        if (ImGui::TreeNode("Images")) {
            ...
            f32 my_tex_w = (f32) io.Fonts->TexWidth;
            f32 my_tex_h = (f32) io.Fonts->TexHeight;
            ...
            if (ImGui::IsItemHovered()) {
                ImGui::BeginTooltip();
                f32 region_sz = 32.0f;
                f32 region_x = io.MousePos.x - pos.x - region_sz * 0.5f; if (region_x < 0.0f) region_x = 0.0f; else if (region_x > my_tex_w - region_sz) region_x = my_tex_w - region_sz;
                f32 region_y = io.MousePos.y - pos.y - region_sz * 0.5f; if (region_y < 0.0f) region_y = 0.0f; else if (region_y > my_tex_h - region_sz) region_y = my_tex_h - region_sz;
                f32 zoom = 4.0f;
        ...
        if (ImGui::TreeNode("Combo")) {
            ...
            char const *items[] = { "AAAA", "BBBB", "CCCC", "DDDD", "EEEE", "FFFF", "GGGG", "HHHH", "IIII", "JJJJ", "KKKK", "LLLLLLL", "MMMM", "OOOOOOO" };
            static char const *item_current = items[0];            // Here our selection is a single pointer stored outside the object.
        ...
        if (ImGui::TreeNode("Selectables")) {
            ...
            if (ImGui::TreeNode("Basic")) {
                ...
                if (ImGui::Selectable("5. I am f64 clickable", selection[4], ImGuiSelectableFlags_AllowDoubleClick))
        ...
        if (ImGui::TreeNode("Plots Widgets")) {
            ...
            // Create a dummy array of contiguous f32 values to plot
            // Tip: If your f32 aren't contiguous but part of a structure, you can pass a pointer to your first f32 and the sizeof() of your structure in the Stride parameter.
            static f32 values[90] = { 0 };
            ...
            // FIXME: This is rather awkward because current plot API only pass in indices. We probably want an API passing f32s and user provide sample rate/count.
            struct Funcs {
                static f32 Sin(void*, int i) { return sinf(i * 0.1f); }
                static f32 Saw(void*, int i) { return (i & 1) ? 1.0f : -1.0f; }
            };
            ...
            f32(*func)(void*, int) = (func_type == 0) ? Funcs::Sin : Funcs::Saw;
            ImGui::PlotLines("Lines", func, NULL, display_count, 0, NULL, -1.0f, 1.0f, ImVec2(0, 80));
            ImGui::PlotHistogram("Histogram", func, NULL, display_count, 0, NULL, -1.0f, 1.0f, ImVec2(0, 80));
            ImGui::Separator();

            // Animate a simple progress bar
            static f32 progress = 0.0f, progress_dir = 1.0f;
            if (animate) {
            ...
            f32 progress_saturated = (progress < 0.0f) ? 0.0f : (progress > 1.0f) ? 1.0f : progress;
            char buf[32];
        ...
        if (ImGui::TreeNode("Color/Picker Widgets")) {
            ...
            ImGui::SameLine(); ShowHelpMarker("Click on the colored square to open a color picker.\nCTRL+click on individual component to input value.\n");
            ImGui::ColorEdit3("MyColor##1", (f32*) &color, misc_flags);

            ImGui::Text("Color widget HSV with Alpha:");
            ImGui::ColorEdit4("MyColor##2", (f32*) &color, ImGuiColorEditFlags_HSV | misc_flags);

            ImGui::Text("Color widget with Float Display:");
            ImGui::ColorEdit4("MyColor##2f", (f32*) &color, ImGuiColorEditFlags_Float | misc_flags);

            ImGui::Text("Color button with Picker:");
            ImGui::SameLine(); ShowHelpMarker("With the ImGuiColorEditFlags_NoInputs flag you can hide all the slider/text inputs.\nWith the ImGuiColorEditFlags_NoLabel flag you can pass a non-empty label which will only be used for the tooltip and picker popup.");
            ImGui::ColorEdit4("MyColor##3", (f32*) &color, ImGuiColorEditFlags_NoInputs | ImGuiColorEditFlags_NoLabel | misc_flags);
            ...
            ImGui::ColorPicker4("MyColor##4", (f32*) &color, flags, ref_color ? &ref_color_v.x : NULL);
        ...
        if (ImGui::TreeNode("Data Types")) {
            // The DragScalar, InputScalar, SliderScalar functions allow manipulating most common data types: signed/unsigned int/long long and f32/f64
            ...
            //   bool SliderU64(char const *label, u64* value, u64 min = 0, u64 max = 0, char const *format = "%lld") { return SliderScalar(label, ImGuiDataType_U64, value, &min, &max, format); }
            ...
            const f32   f32_zero = 0.f, f32_one = 1.f, f32_lo_a = -10000000000.0f, f32_hi_a = +10000000000.0f;
            const f64  f64_zero = 0., f64_one = 1., f64_lo_a = -1000000000000000, f64_hi_a = +1000000000000000;
            ...
            static f32  f32_v = 0.123f;
            static f64 f64_v = 90000.01234567890123456789;

            const f32 drag_speed = 0.2f;
            ...
            ImGui::DragScalar("drag f32", ImGuiDataType_Float, &f32_v, 0.005f, &f32_zero, &f32_one, "%f", 1.0f);
            ImGui::DragScalar("drag f32 ^2", ImGuiDataType_Float, &f32_v, 0.005f, &f32_zero, &f32_one, "%f", 2.0f); ImGui::SameLine(); ShowHelpMarker("You can use the 'power' parameter to increase tweaking precision on one side of the range.");
            ImGui::DragScalar("drag f64", ImGuiDataType_Double, &f64_v, 0.0005f, &f64_zero, NULL, "%.10f grams", 1.0f);
            ImGui::DragScalar("drag f64 ^2", ImGuiDataType_Double, &f64_v, 0.0005f, &f64_zero, &f64_one, "0 < %.10f < 1", 2.0f);
            ...
            ImGui::SliderScalar("slider f32 low", ImGuiDataType_Float, &f32_v, &f32_zero, &f32_one);
            ImGui::SliderScalar("slider f32 low^2", ImGuiDataType_Float, &f32_v, &f32_zero, &f32_one, "%.10f", 2.0f);
            ImGui::SliderScalar("slider f32 high", ImGuiDataType_Float, &f32_v, &f32_lo_a, &f32_hi_a, "%e");
            ImGui::SliderScalar("slider f64 low", ImGuiDataType_Double, &f64_v, &f64_zero, &f64_one, "%.10f grams", 1.0f);
            ImGui::SliderScalar("slider f64 low^2", ImGuiDataType_Double, &f64_v, &f64_zero, &f64_one, "%.10f", 2.0f);
            ImGui::SliderScalar("slider f64 high", ImGuiDataType_Double, &f64_v, &f64_lo_a, &f64_hi_a, "%e grams", 1.0f);
            ...
            ImGui::InputScalar("input f32", ImGuiDataType_Float, &f32_v, inputs_step ? &f32_one : NULL);
            ImGui::InputScalar("input f64", ImGuiDataType_Double, &f64_v, inputs_step ? &f64_one : NULL);

            ImGui::TreePop();
        }

        if (ImGui::TreeNode("Multi-component Widgets")) {
            static f32 vec4f[4] = { 0.10f, 0.20f, 0.30f, 0.44f };
            static int vec4i[4] = { 1, 5, 100, 255 };

            ImGui::InputFloat2("input f322", vec4f);
            ImGui::DragFloat2("drag f322", vec4f, 0.01f, 0.0f, 1.0f);
            ImGui::SliderFloat2("slider f322", vec4f, 0.0f, 1.0f);
            ImGui::InputInt2("input int2", vec4i);
            ImGui::DragInt2("drag int2", vec4i, 1, 0, 255);
            ImGui::SliderInt2("slider int2", vec4i, 0, 255);
            ImGui::Spacing();

            ImGui::InputFloat3("input f323", vec4f);
            ImGui::DragFloat3("drag f323", vec4f, 0.01f, 0.0f, 1.0f);
            ImGui::SliderFloat3("slider f323", vec4f, 0.0f, 1.0f);
            ImGui::InputInt3("input int3", vec4i);
            ImGui::DragInt3("drag int3", vec4i, 1, 0, 255);
            ImGui::SliderInt3("slider int3", vec4i, 0, 255);
            ImGui::Spacing();

            ImGui::InputFloat4("input f324", vec4f);
            ImGui::DragFloat4("drag f324", vec4f, 0.01f, 0.0f, 1.0f);
            ImGui::SliderFloat4("slider f324", vec4f, 0.0f, 1.0f);
        ...
        if (ImGui::TreeNode("Vertical Sliders")) {
            ...
            static f32 values[7] = { 0.0f, 0.60f, 0.35f, 0.9f, 0.70f, 0.20f, 0.0f };
            ImGui::PushID("set1");
            for (int i = 0; i < 7; i++) {
                if (i > 0) ImGui::SameLine();
                ImGui::PushID(i);
            ...
            ImGui::SameLine();
            ImGui::PushID("set2");
            static f32 values2[4] = { 0.20f, 0.80f, 0.40f, 0.25f };
            const int rows = 3;
        ...
        if (ImGui::TreeNode("Drag and Drop")) {
...

            {
                ...
                static char const *names[9] = { "Bobby", "Beatrice", "Betty", "Brianna", "Barry", "Bernard", "Bibi", "Blaine", "Bryn" };
        ...
        if (ImGui::TreeNode("Active, Focused, Hovered & Focused Tests")) {
            ...
            static bool b = false;
            static f32 col4f[4] = { 1.0f, 0.5, 0.0f, 1.0f };
            ...
            if (item_type == 5) { char const *items[] = { "Apple", "Banana", "Cherry", "Kiwi" }; static int current = 1; ret = ImGui::ListBox("ITEM: ListBox", &current, items, IM_ARRAYSIZE(items), IM_ARRAYSIZE(items)); }
            ImGui::BulletText(
                "Return value = %d\n"
                "IsItemFocused() = %d\n"
                "IsItemHovered() = %d\n"
    ...
    if (ImGui::CollapsingHeader("Layout")) {
        ...
        if (ImGui::TreeNode("Widgets Width")) {
            ...
            ImGui::PushItemWidth(100);
            ImGui::DragFloat("f32##1", &f);
            ImGui::PopItemWidth();

            ImGui::Text("PushItemWidth(GetWindowWidth() * 0.5f)");
            ImGui::SameLine(); ShowHelpMarker("Half of window width.");
            ImGui::PushItemWidth(ImGui::GetWindowWidth() * 0.5f);
            ImGui::DragFloat("f32##2", &f);
            ImGui::PopItemWidth();

            ImGui::Text("PushItemWidth(GetContentRegionAvailWidth() * 0.5f)");
            ImGui::SameLine(); ShowHelpMarker("Half of available width.\n(~ right-cursor_pos)\n(works within a column set)");
            ImGui::PushItemWidth(ImGui::GetContentRegionAvailWidth() * 0.5f);
            ImGui::DragFloat("f32##3", &f);
            ImGui::PopItemWidth();

            ImGui::Text("PushItemWidth(-100)");
            ImGui::SameLine(); ShowHelpMarker("Align to right edge minus 100");
            ImGui::PushItemWidth(-100);
            ImGui::DragFloat("f32##4", &f);
            ImGui::PopItemWidth();

            ImGui::Text("PushItemWidth(-1)");
            ImGui::SameLine(); ShowHelpMarker("Align to right edge");
            ImGui::PushItemWidth(-1);
            ImGui::DragFloat("f32##5", &f);
            ImGui::PopItemWidth();
        ...
        if (ImGui::TreeNode("Basic Horizontal Layout")) {
            ...
            // Various
            static f32 f0=1.0f, f1=2.0f, f2=3.0f;
            ImGui::PushItemWidth(80);
            char const *items[] = { "AAAA", "BBBB", "CCCC", "DDDD" };
            static int item = -1;
        ...
        if (ImGui::TreeNode("Groups")) {
            ...
            const f32 values[5] = { 0.5f, 0.20f, 0.80f, 0.60f, 0.25f };
        ...
        if (ImGui::TreeNode("Scrolling")) {
            ...
            for (int i = 0; i < 5; i++) {
                ...
                f32 scroll_y = ImGui::GetScrollY(), scroll_max_y = ImGui::GetScrollMaxY();
                ImGui::EndChild();
        ...
        if (ImGui::TreeNode("Horizontal Scrolling")) {
            ...
            for (int line = 0; line < lines; line++) {
                ...
                for (int n = 0; n < num_buttons; n++) {
                    ...
                    char num_buf[16];
                    sprintf(num_buf, "%d", n);
                    char const *label = (!(n%15)) ? "FizzBuzz" : (!(n%3)) ? "Fizz" : (!(n%5)) ? "Buzz" : num_buf;
                    f32 hue = n*0.05f;
                    ImGui::PushStyleColor(ImGuiCol_Button, (ImVec4) ImColor::HSV(hue, 0.6f, 0.6f));
                    ImGui::PushStyleColor(ImGuiCol_ButtonHovered, (ImVec4) ImColor::HSV(hue, 0.7f, 0.7f));
                    ImGui::PushStyleColor(ImGuiCol_ButtonActive, (ImVec4) ImColor::HSV(hue, 0.8f, 0.8f));
                    ImGui::Button(label, ImVec2(40.0f + sinf((f32) (line + n)) * 20.0f, 0.0f));
                    ImGui::PopStyleColor(3);
                    ImGui::PopID();
                }
            }
            f32 scroll_x = ImGui::GetScrollX(), scroll_max_x = ImGui::GetScrollMaxX();
            ImGui::EndChild();
            ImGui::PopStyleVar(2);
            f32 scroll_x_delta = 0.0f;
        ...
        if (ImGui::TreeNode("Clipping")) {
            ...
            ImGui::DragFloat2("size", (f32*) &size, 0.5f, 0.0f, 200.0f, "%.0f");
    ...
    if (ImGui::CollapsingHeader("Popups & Modal windows")) {
        if (ImGui::TreeNode("Popups")) {
            ...
            char const *names[] = { "Bream", "Haddock", "Mackerel", "Pollock", "Tilefish" };
        ...
        if (ImGui::TreeNode("Modals")) {
            ...
            if (ImGui::BeginPopupModal("Stacked 1")) {
                ...
                static f32 color[4] = { 0.4f,0.7f,0.0f,0.5f };
    ...
    if (ImGui::CollapsingHeader("Columns")) {
        ...
        if (ImGui::TreeNode("Basic")) {
            ...
            ImGui::Separator();
            char const *names[3] = { "One", "Two", "Three" };
            char const *paths[3] = { "/path/one", "/path/two", "/path/three" };
            static int selected = -1;
            for (int i = 0; i < 3; i++) {
                char label[32];
                sprintf(label, "%04d", i);
    ...
    if (ImGui::CollapsingHeader("Filtering")) {
        ...
        filter.Draw();
        char const *lines[] = { "aaa1.c", "bbb1.c", "ccc1.c", "aaa2.cpp", "bbb2.cpp", "ccc2.cpp", "abc.h", "hello, world" };
    ...
    if (ImGui::CollapsingHeader("Inputs, Navigation & Focus")) {
        ...
        if (ImGui::TreeNode("Focus from code")) {
            ...
            static f32 f3[3] = { 0.0f, 0.0f, 0.0f };
            int focus_ahead = -1;
        ...
        if (ImGui::TreeNode("Mouse cursors")) {
            char const *mouse_cursors_names[] = { "Arrow", "TextInput", "Move", "ResizeNS", "ResizeEW", "ResizeNESW", "ResizeNWSE", "Hand" };
...
bool ImGui::ShowStyleSelector(char const *label) {
...
void ImGui::ShowFontSelector(char const *label) {
    ImGuiIO& io = ImGui::GetIO();
...
void ImGui::ShowStyleEditor(ImGuiStyle* ref) {
    ...
    if (ImGui::TreeNode("Settings")) {
        ImGui::SliderFloat2("WindowPadding", (f32*) &style.WindowPadding, 0.0f, 20.0f, "%.0f");
        ImGui::SliderFloat("PopupRounding", &style.PopupRounding, 0.0f, 16.0f, "%.0f");
        ImGui::SliderFloat2("FramePadding", (f32*) &style.FramePadding, 0.0f, 20.0f, "%.0f");
        ImGui::SliderFloat2("ItemSpacing", (f32*) &style.ItemSpacing, 0.0f, 20.0f, "%.0f");
        ImGui::SliderFloat2("ItemInnerSpacing", (f32*) &style.ItemInnerSpacing, 0.0f, 20.0f, "%.0f");
        ImGui::SliderFloat2("TouchExtraPadding", (f32*) &style.TouchExtraPadding, 0.0f, 10.0f, "%.0f");
        ...
        ImGui::SliderFloat2("WindowTitleAlign", (f32*) &style.WindowTitleAlign, 0.0f, 1.0f, "%.2f");
        ImGui::SliderFloat2("ButtonTextAlign", (f32*) &style.ButtonTextAlign, 0.0f, 1.0f, "%.2f"); ImGui::SameLine(); ShowHelpMarker("Alignment applies when a button is larger than its text content.");
        ...
        ImGui::SliderFloat2("DisplaySafeAreaPadding", (f32*) &style.DisplaySafeAreaPadding, 0.0f, 30.0f, "%.0f");
        ImGui::TreePop();
    }
    ...
    if (ImGui::TreeNode("Colors")) {
        ...
        for (int i = 0; i < ImGuiCol_COUNT; i++) {
            ...
            if (!filter.PassFilter(name))
                continue;
            ImGui::PushID(i);
            ImGui::ColorEdit4("##color", (f32*) &style.Colors[i], ImGuiColorEditFlags_AlphaBar | alpha_flags);
    ...
    if (fonts_opened) {
        ...
        if (ImGui::TreeNode("Atlas texture", "Atlas texture (%dx%d pixels)", atlas->TexWidth, atlas->TexHeight)) {
            ImGui::Image(atlas->TexID, ImVec2((f32) atlas->TexWidth, (f32) atlas->TexHeight), ImVec2(0, 0), ImVec2(1, 1), ImColor(255, 255, 255, 255), ImColor(255, 255, 255, 128));
            ImGui::TreePop();
        }
        ImGui::PushItemWidth(100);
        for (int i = 0; i < atlas->Fonts.Size; i++) {
            ...
            if (font_details_opened) {
                ...
                ImGui::Text("Texture surface: %d pixels (approx) ~ %dx%d", font->MetricsTotalSurface, (int) sqrtf((f32) font->MetricsTotalSurface), (int) sqrtf((f32) font->MetricsTotalSurface));
                ...
                if (ImGui::TreeNode("Glyphs", "Glyphs (%d)", font->Glyphs.Size)) {
                    ...
                    for (int base = 0; base < 0x10000; base += 256) {
                        ...
                        if (count > 0 && ImGui::TreeNode((void*) (intptr_t) base, "U+%04X..U+%04X (%d %s)", base, base+255, count, count > 1 ? "glyphs" : "glyph")) {
                            f32 cell_size = font->FontSize * 1;
                            f32 cell_spacing = style.ItemSpacing.y;
...
static void ShowExampleMenuFile() {
    ...
    if (ImGui::BeginMenu("Colors")) {
        ...
        for (int i = 0; i < ImGuiCol_COUNT; i++) {
            char const *name = ImGui::GetStyleColorName((ImGuiCol) i);
...
struct ExampleAppConsole {
    ...
    // Portable helpers
    static int   Stricmp(char const *str1, char const *str2) { int d; while ((d = toupper(*str2) - toupper(*str1)) == 0 && *str1) { str1++; str2++; } return d; }
    static int   Strnicmp(char const *str1, char const *str2, int n) { int d = 0; while (n > 0 && (d = toupper(*str2) - toupper(*str1)) == 0 && *str1) { str1++; str2++; n--; } return d; }
    static char* Strdup(char const *str) { size_t len = strlen(str) + 1; void* buff = malloc(len); return (char*) memcpy(buff, (const void*) str, len); }
    ...
    void    Draw(char const *title, bool* p_open) {
        ...
        //static f32 t = 0.0f; if (ImGui::GetTime() - t > 0.02f) { t = ImGui::GetTime(); AddLog("Spam %f", t); }
        ...
        const f32 footer_height_to_reserve = ImGui::GetStyle().ItemSpacing.y + ImGui::GetFrameHeightWithSpacing(); // 1 separator, 1 input text
    ...
    void    ExecCommand(char const *command_line) {
        AddLog("# %s\n", command_line);
    ...
    void    Draw(char const *title, bool* p_open = NULL) {
        ...
        if (Filter.IsActive()) {
            ...
            for (int line_no = 0; line != NULL; line_no++) {
                char const *line_end = (line_no < LineOffsets.Size) ? buf_begin + LineOffsets[line_no] : NULL;
...
static void ShowExampleAppLog(bool* p_open) {
    ...
    static f64 last_time = -1.0;
    f64 time = ImGui::GetTime();
    if (time - last_time >= 0.20f && !ImGui::GetIO().KeyCtrl) {
        char const *random_words[] = { "system", "info", "warning", "error", "fatal", "notice", "log" };
...
static void ShowExampleAppPropertyEditor(bool* p_open) {
    ...
    struct funcs {
        static void ShowDummyObject(char const *prefix, int uid) {
            ...
            ImGui::NextColumn();
            if (node_open) {
                static f32 dummy_members[8] = { 0.0f,0.0f,1.0f,3.1416f,100.0f,999.0f };
                for (int i = 0; i < 8; i++) {
...
static void ShowExampleAppConstrainedResize(bool* p_open) {
    struct CustomConstraints // Helper functions to demonstrate programmatic constraints
    {
        ...
        static void Step(ImGuiSizeCallbackData* data) { f32 step = (f32) (int) (intptr_t) data->UserData; data->DesiredSize = ImVec2((int) (data->DesiredSize.x / step + 0.5f) * step, (int) (data->DesiredSize.y / step + 0.5f) * step); }
    };
...
static void ShowExampleAppCustomRendering(bool* p_open) {
    ...
    ImGui::ColorEdit3("Color", &col.x);
    {
        ...
        f32 x = p.x + 4.0f, y = p.y + 4.0f, spacing = 8.0f;
        for (int n = 0; n < 2; n++) {
            f32 curr_thickness = (n == 0) ? 1.0f : thickness;
