...
def handle_file(path, file_name):
    global build_test_table_contents, included_files
    ...
    #
    if ":IgnoreTestFile:" in contents: 
        return
    
    # Add the file to included files set and include it
    if path not in included_files:
        included_files.add(path)

    build_test_table_contents += '    {\n'
    build_test_table_contents += f'        auto [_, array] = add(g_TestTable, string("{file_name}"), {{}});\n'
    build_test_table_contents += '        reserve(*array, 10);\n'
...
# Generate include statements for all test files
include_statements = ""
for file_path in sorted(included_files):
    # Convert absolute path to relative path from test-suite directory
    relative_path = os.path.relpath(file_path, ".")
    include_statements += f'#include "{relative_path}"\n'

file_content = (''
'#include "test.h"\n'
'\n'
f'{include_statements}'
'\n'
'//\n'
