...
static void thread_lock_free(void *) {
  For(range(10000)) { atomic_inc(&Count); }
}
...
TEST(lock_free) {
  ...
  For(range(100)) { add(threads, create_and_launch_thread(thread_lock_free)); }
...
TEST(mutex_lock) {
  ...
  For(range(100)) { add(threads, create_and_launch_thread(thread_lock)); }
...
TEST(fast_mutex_lock) {
  ...
  For(range(100)) { add(threads, create_and_launch_thread(thread_lock2)); }
...
TEST(context) {
  ...
  PUSH_ALLOC(differentAlloc) {
    auto threadFunction = [&](void *) {
      assert_eq((void *)Context.Alloc.Function,
                (void *)differentAlloc.Function);
      []() {
        PUSH_ALLOC(TemporaryAllocator) {
          assert_eq((void *)Context.Alloc.Function,
                    (void *)TemporaryAllocator.Function);
          return;
        }
      }();
      assert_eq((void *)Context.Alloc.Function,
