...
TEST(benchmark_string) { CHECK_WRITE("0.1250000000:0042:+0.25:str:0x3e8:X:%", "{0:0.10f}:{1:04}:{2:+g}:{3}:{4}:{5:c}:%", 0.125, 42, 0.25, "str", (void *)1000, 'X'); }
...
TEST(nested_hash_tables_pretty) {
    ...
    hash_table<string, s32> inner1;
    defer(free(inner1));
    set(inner1, "apple", 5);
    set(inner1, "banana", 3);

    hash_table<string, s32> inner2;
    defer(free(inner2));
...
TEST(array_of_hash_tables_pretty_smoke) {
    ...
    string s1 = sprint("{}", testFmt);
    defer(free(s1.Data));
    assert(s1.Count && s1[0] == '[');

    string s2 = sprint("{:2}", testFmt);
    defer(free(s2.Data));
    assert(s2.Count && s2[0] == '[');

    string s3 = sprint("{:#2}", testFmt);
    defer(free(s3.Data));
