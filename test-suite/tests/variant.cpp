...
TEST(variant_visit_pattern_matching) {
    ...
    v = 42;
    string result;
    v.visit(match{[&result](int x) { result = sprint("int: {}", x); }, [&result](float x) { result = sprint("float: {}", x); },
                  [&result](string x) { result = sprint("string: {}", x); }, [&result](auto) { result = "empty"; }});
...
TEST(variant_const_operations) {
    ...
    // Test const visit
    string result;
    v.visit(match{[&result](int x) { result = sprint("const int: {}", x); }, [&result](string x) { result = sprint("const string: {}", x); },
...
TEST(optional_with_string) {
    ...
    string result;
    opt.visit(match{[&result](string s) { result = sprint("got: {}", s); }, [&result](auto) { result = "empty"; }});
...
TEST(variant_complex_types) {
    ...
    v.visit(match{[&found_point](point p) { found_point = (p.x == 10 && p.y == 20); }, [](auto) {}});
...
TEST(variant_with_array_and_string) {
    ...
    // Visit both cases
    string res;
    v.visit(match{[&res](array<s32> ar2) { res = sprint("arr:{}", ar2.Count); }, [&res](string s) { res = sprint("str:{}", s); }, [&res](auto) { res = "empty"; }});
    assert_eq_str(res, "str:abc");

    // Make variant empty and visit default branch
    v = variant<array<s32>, string>{};
    v.visit(match{[](array<s32>) {}, [](string) {}, [&res](auto) { res = "empty"; }});
