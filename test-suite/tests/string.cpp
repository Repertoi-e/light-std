...
TEST(iterator) {
    ...
    for (auto ch : b) { ch = unicode_to_lower(ch); }
    assert_eq_str(b, "hello");
    for (auto ch : b) { ch = U'Д'; }
...
TEST(lower_upper) {
    ...
    for (auto ch : lower) { ch = unicode_to_lower(ch); }
    assert_eq_str(lower, u8"abcдеж");

    string upper = clone(a);
    defer(free(upper));

    for (auto ch : upper) { ch = unicode_to_upper(ch); }
